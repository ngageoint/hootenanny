...
# installation
# - is meant to be run from a user's home directory
#
...
REMOVE_EXISTING_CORE_INSTALL=false
...
if [ "$UPDATE_CORE" == "true" ]; then
  ...
  # building the docs requires git to generate dates, which we don't have...so just backing up
  # the already included docs and redeploying them instead
  sudo cp -R docs $TEMP_DIR
  source ./SetupEnv.sh
  touch conf/DatabaseConfig.sh
  ./configure -q --prefix=$HOOT_CORE_INSTALL_DIR/hootenanny-$NEW_VERSION $CORE_CONFIGURATION_OPTIONS
  #make -s clean-all
...
if [ "$UPDATE_FOUO_TRANSLATIONS" == "true" ]; then
  ...
  tar -xzf hootenanny-translations-$NEW_VERSION.tar.gz > /dev/null
