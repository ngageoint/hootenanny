...
cd cppunit-1.12.1
./configure -q --prefix=$HOOT_DEST/ && make -j8 && sudo make install
cd ..
...
if [ "$INSTALLING_FROM_VIRTUALBOX_SHARED_FOLDER" == "true" ]; then
  ...
  rm -rf $HOOT_DEST/opencv-2.4.5
  cp -R opencv-2.4.5 $HOOT_DEST
  cd $HOOT_DEST/opencv-2.4.5
else
  cd opencv-2.4.5
fi
mkdir -p release
...
#./configure --prefix=$HOOT_DEST && make -j8 && sudo make install
#cd ..
...
test -f asciidoc-8.6.9.tar.gz || wget http://sourceforge.net/projects/asciidoc/files/asciidoc/8.6.9/asciidoc-8.6.9.tar.gz/download
if [ "$INSTALLING_FROM_VIRTUALBOX_SHARED_FOLDER" == "true" ]; then
  # for whatever reason you can't install this from a virtualbox shared folder
  rm -rf $HOOT_DEST/asciidoc-8.6.9
  cp asciidoc-8.6.9.tar.gz $HOOT_DEST
  cd $HOOT_DEST
...
tar -xzf asciidoc-8.6.9.tar.gz
cd asciidoc-8.6.9
autoconf && ./configure --prefix=$HOOT_DEST && make -j8 && sudo make install
cd ..
...
cd dblatex-0.2.8
sudo python setup.py install --prefix=$HOOT_DEST
cd ..
...
export PYTHONPATH=$INSTALL_DIR/matplotlib-1.3.1
rm -rf matplotlib-1.3.1
test -f matplotlib-1.3.1.tar.gz || wget http://sourceforge.net/projects/matplotlib/files/matplotlib/matplotlib-1.3.1/matplotlib-1.3.1.tar.gz/download
tar -xzf matplotlib-1.3.1.tar.gz
cd matplotlib-1.3.1
python setup.py build && sudo python setup.py install #--prefix=$HOOT_DEST
cd ..
...
sudo rm -rf $HOOT_DEST/FileGDB_API-1.3
sudo tar -xzf FileGDB_API_1_3-64.tar.gz --directory /usr/local
sudo cp -R FileGDB_API $HOOT_DEST/FileGDB_API-1.3
...
cd gdal-1.10.1
./configure -q --with-pg=/usr/pgsql-9.1/bin/pg_config --with-fgdb=$HOOT_DEST/FileGDB_API/ --with-python --prefix=$HOOT_DEST && make -j8 && sudo make install
cd swig/python
...
sudo rm -f $HOOT_DEST/qt-everywhere-opensource-src-4.8.6.tar.gz
rm -rf qt-everywhere-opensource-src-4.8.6
sudo rm -rf $HOOT_DEST/qt-everywhere-opensource-src-4.8.6
...
sudo cp qt-everywhere-opensource-src-4.8.6.tar.gz $HOOT_DEST
cd $HOOT_DEST
sudo tar -xzf qt-everywhere-opensource-src-4.8.6.tar.gz
cd qt-everywhere-opensource-src-4.8.6
./configure -opensource -prefix /usr/lib64/qt4 -no-opengl -no-xvideo -no-xinput -no-xkb -no-cups -no-webkit -no-qt3support -fast -plugin-sql-psql -I/usr/pgsql-9.1/include -L/usr/pgsql-9.1/lib -nomake demos -nomake examples -confirm-license -silent --prefix=$HOOT_DEST && make -j8 && sudo make install
cd $INSTALL_DIR
sudo rm -f /usr/bin/qmake
sudo ln -s /usr/lib64/qt4/qmake /usr/bin/qmake
if ! grep --quiet QT_HOME /etc/profile; then
  #TODO: replace /usr/local with $HOOT_DEST
...
#TODO: use $HOOT_DEST here instead of /usr/local
...
echo "Hootenanny dependencies have been installed.  You can install the Hootenanny application with "
echo "hootenanny-X.Y.Z/scripts/UpdateHoot-CentOS-RHEL.sh"
