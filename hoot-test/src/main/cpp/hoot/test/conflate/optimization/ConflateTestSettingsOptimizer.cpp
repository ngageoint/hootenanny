...
void ConflateTestSettingsOptimizer::runOptimization(std::shared_ptr<AbstractTestFitnessFunction> fitnessFunction,
                                                    const int numIterations, const QString& testSettingsFile,
                                                 ...
                                                 const QString& outputFile)
{
  ...
  temp = "Lowest number of test failures in test iteration: " +
  ...
  output += "Failing Test Groups For Best States (" + QString::number(failingTestsForBestRuns.size()) +
            "):\n\n";
  ...
  int statesCtr = 1;
  for (auto state : qAsConst(bestStates))
  {
