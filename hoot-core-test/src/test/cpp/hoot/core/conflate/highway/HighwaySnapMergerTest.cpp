...
public:
  ...
  void runTagsTest()
  {
    ...
    QString expected = QString("{'version': 0.6,'generator': 'Hootenanny','elements': [\n"
        ...
        "{'type':'way','id':-17,'nodes':[-5,-1],'tags':{'highway':'path','uuid':'w3'},\n"
        "{'type':'way','id':-14,'nodes':[-11,-4],'tags':{'barrier':'wall','uuid':'wall'},\n"
        "{'type':'way','id':-13,'nodes':[-3,-11],'tags':{'barrier':'wall','uuid':'wall'},\n"
        "{'type':'way','id':-12,'nodes':[-1,-2],'tags':{'uuid':'w1'},\n"
        "{'type':'relation','id':-2,'members':[\n"
        "{'type':'way','ref':-14,'role':''}],'tags':{'highway':'footway','uuid':'r'},\n"
        "{'type':'relation','id':-1,'members':[\n"
        "{'type':'way','ref':-12,'role':''},\n"
        "{'type':'way','ref':-13,'role':''}],'tags':{'highway':'footway','uuid':'r;w3'}]\n"
        "}\n"
        "").replace("'", "\"");
  ...
  void runTagsSplitTest()
  {
    ...
    QString expected = QString(
      ...
      "{'type':'way','id':-6,'nodes':[-8,-2],,\n"
      "{'type':'way','id':-5,'nodes':[-1,-7],,\n"
      "{'type':'way','id':-4,'nodes':[-7,-8],'tags':{'highway':'road','uuid':'w1;w2'},\n"
      "{'type':'relation','id':-1,'members':[\n"
      "{'type':'way','ref':-5,'role':''},\n"
      "{'type':'way','ref':-6,'role':''}],'tags':{'highway':'road','uuid':'w1'}]\n"
      "}\n"
      "").replace("'", "\"");
    HOOT_STR_EQUALS(expected, json);
  ...
  void runTagsNoRelationTest()
  {
    ...
    QString expected = QString(
      ...
      "{'type':'way','id':-6,'nodes':[-2,-4],'tags':{'highway':'footway','uuid':'r'},\n"
      "{'type':'way','id':-4,'nodes':[-1,-2],'tags':{'highway':'road','uuid':'w1;r'}]\n"
      "}\n"
      "").replace("'", "\"");
    HOOT_STR_EQUALS(expected, json);
  }
