...
public:
  ...
  void runBasicTest()
  {
    ...
    uut = std::make_shared<WayLengthCriterion>(-1.0, NumericComparisonType::LessThanOrEqualTo, map);
    way = std::make_shared<Way>(Status::Unknown1, 1);
    map->addWay(way);
    ...
    uut = std::make_shared<WayLengthCriterion>(0.0, NumericComparisonType::EqualTo, map);
    CPPUNIT_ASSERT(!uut->isSatisfied(way));
  }
