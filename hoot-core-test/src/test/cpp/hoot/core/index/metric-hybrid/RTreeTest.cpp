...
public:
  ...
  void runRandomQueryTest()
  {
    ...
    {
      double x = rand() / (double)RAND_MAX * 100.0;
      double y = rand() / (double)RAND_MAX * 100.0;
    ...
    for (int depth = 0; depth <= 6; depth+=2)
    {
      ...
      {
        double x = rand() / (double)RAND_MAX * 300.0 - 150.0;
        double y = rand() / (double)RAND_MAX * 300.0 - 150.0;
        Coordinate c(x, y);
