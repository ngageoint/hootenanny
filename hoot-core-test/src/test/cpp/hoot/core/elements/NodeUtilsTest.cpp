...
public:
  ...
  void runNodeIdsTest()
  {
    ...
    for (auto nodeIter = mapNodes.begin(); nodeIter != mapNodes.end(); ++nodeIter)
      nodes.append(nodeIter->second);

    CPPUNIT_ASSERT_EQUAL(36, NodeUtils::nodesToNodeIds(nodes).size());
    CPPUNIT_ASSERT_EQUAL((size_t)36, NodeUtils::nodesToNodeIds(nodes.toVector().toStdVector()).size());
    ...
    CPPUNIT_ASSERT_EQUAL((size_t)36, NodeUtils::nodeIdsToNodes(QList<long>::fromSet(nodeIds).toVector().toStdVector(), map).size());
  }

  void runCoordsMatchTest()
  {
    std::vector<ConstNodePtr> nodes1;
    ConstNodePtr node1 = std::make_shared<Node>(Status::Unknown1, 1, geos::geom::Coordinate(0.0, 0.0), 15.0);
    nodes1.push_back(node1);
    ConstNodePtr node2 = std::make_shared<Node>(Status::Unknown1, 2, geos::geom::Coordinate(0.0, 5.0), 15.0);
    nodes1.push_back(node2);
    std::vector<ConstNodePtr> nodes2;
    ConstNodePtr node3 = std::make_shared<Node>(Status::Unknown1, 3, geos::geom::Coordinate(0.0, 0.0), 15.0);
    nodes2.push_back(node3);
    ConstNodePtr node4 = std::make_shared<Node>(Status::Unknown1, 4, geos::geom::Coordinate(0.0, 5.0), 15.0);
    nodes2.push_back(node4);
    CPPUNIT_ASSERT(NodeUtils::nodeCoordsMatch(nodes1, nodes2));

    std::vector<ConstNodePtr> nodes3;
    ConstNodePtr node5 = std::make_shared<Node>(Status::Unknown1, 5, geos::geom::Coordinate(0.0, 0.0), 15.0);
    nodes3.push_back(node5);
    ConstNodePtr node6 = std::make_shared<Node>(Status::Unknown1, 6, geos::geom::Coordinate(0.0, 10.0), 15.0);
    nodes3.push_back(node6);
    CPPUNIT_ASSERT(!NodeUtils::nodeCoordsMatch(nodes1, nodes3));

    std::vector<ConstNodePtr> nodes4;
    ConstNodePtr node7 = std::make_shared<Node>(Status::Unknown1, 7, geos::geom::Coordinate(0.0, 0.0), 15.0);
    ...
    ConstNodePtr node9 = std::make_shared<Node>(Status::Unknown1, 9, geos::geom::Coordinate(0.0, 0.0), 15.0);
