...
void DataFrame::addDataVector(const std::string& label, const std::vector<double>& dataItem,
...
double DataFrame::computeBandwidthByFactor(unsigned int fIdx, std::vector<unsigned int> & dataIndices,
                                           double & minVal, double & maxVal, double & mean,
                                           double & q1, double & q3)
{
  try
  {
    if (!dataIndices.empty() && fIdx < _factorLabels.size())
    {
      ...
      {
        sum += _data[index][fIdx];
        sumsqr += _data[index][fIdx] * _data[index][fIdx];
      }
...
void DataFrame::exportData(QDomDocument & modelDoc, QDomElement & dataFrameNode) const
{
  try
  {
    ...
    if (!_factorType.empty())
    {
      ...
      for (size_t i = 0; i < _factorType.size(); i++)
      {
    ...
    if (!_nullTreatment.empty())
    {
      ...
      for (size_t i = 0; i < _nullTreatment.size(); i++)
      {
    ...
    if (!_medianMaps.empty())
    {
      ...
      for (const auto& factorMedianMap : _medianMaps)
      {
                                    ...
                                    HashMap<std::string, int>& populations) const
{
  try
  {
    ...
    {
      if (index < _trainingLabels.size())
        populations[_trainingLabels[index]] += 1;
      else
      {
        std::stringstream ss;
        ss << "Index " << index << " outside of bounds of training label size " <<
...
void DataFrame::makeBalancedRoundRobinBootstrapAndOobSets(const std::string& className1, const std::string& className2,
