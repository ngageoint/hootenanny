...
public:
  ...
  */
  void addDataVector(const std::string& label, const std::vector<double>& dataItem, double eventWeight = 1.0);
  /**
   * Similar to above, but the pointer to dataItem is assumed to be an array of a length equal to
   * getFactorTypes().size().
   */
  void addDataVector(const std::string& label, const double* dataItem, double eventWeight = 1.0);
  ...
  * @param oob  the output container to hold indices to data vectors for the out of bag set
  */
  void makeBalancedRoundRobinBootstrapAndOobSets(const std::string& className1, const std::string& className2,
