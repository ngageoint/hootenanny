...
bool ProbablePathCalculator::_addReturnPaths(const PpPoint& source, const Destination& destination)
{
...
void ProbablePathCalculator::_checkForInconsistency(const PpPoint& p, MyHeap& q)
{
  ...
  for (unsigned int i = 0; i < 8; i++)
  {
    ...
    if (r >= 0 && r < _rows && c >= 0 && c < _cols)
    {
      ...
      if (newCost < f && (newCost < nodes[index].getValue() || nodes[index].getValue() < 0.0))
      {
...
//  }
  ...
  {
    float minD = 1e10f;
    //TODO: The iterator is never used, d and minD stay constant, this is probably wrong
    for (auto it = _destPending.begin(); it != _destPending.end(); ++it)
    {
      float d = _calculateDistance(_toPoint(start), _destinations[0]);
      minD = std::min(minD, d);
    }
...
void ProbablePathCalculator::_randomizeCurrentValues()
{
  ...
  for (auto current : _currentValues)
  {
    if (current >= 0.0)
      _minCost = std::min(_minCost, current);
  }
}
      ...
      if (r >= 0 && r < _rows && c >= 0 && c < _cols)
      {
        ...
        if (_costSurface[dIndex] < 0.0)
        {
          ...
          else
          {
            ...
            if ((nodes[dIndex].getValue() > newCost + h || nodes[dIndex].getValue() < 0.0) && _baseValues[dIndex] >= 0.0)
            {
...
void ProbablePathCalculator::_createCostSurface(const PpPoint& source)
{
  ...
  q.resize(static_cast<int>(_costSurface.size()), -1);
  ...
  _destPending.clear();
  for (auto it = _remainingDest.begin(); it != _remainingDest.end(); ++it)
