...
alternatives --install /usr/bin/psql pgsql-psql "/usr/pgsql-${POSTGRES_VERSION}/bin/psql" 500
...
if ! su postgres -c "psql -lqt" | cut -d \| -f 1 | grep -qw hoot; then
