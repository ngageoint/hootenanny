...
If true and the conditions specified in `railway.one.to.many.identifying.keys` are met, then
matching secondary features will be deleted from conflated output.
...
List of tag keys with values identifying how many tracks a particular rail has (e.g. `lanes`,
`lines`). If any of the tags are found in a rail from the second input, railway matching will
attempt a one to many match from input 2 to input 1 with matches up to the number of tracks
specified in the tag value (arbitarily, the first tag found from the key list is used). Only tag
merging of selected tags (see `railway.one.to.many.transfer.keys`) will be performed with no
geometry merging, regardless of the conflation workflow in use.
...
List of tags to transfer from input 2 rail feature to input 1 rail feature if the conditions
described in `railway.one.to.many.identifying.keys` are satisfied.
...
test cases (`test-files/cases/`) and was originally added to support `MultiaryPoiConflateCmd`.
