...
If true, the OSM API database writer drop the database constraints before writing the data and
re-enable them after the writing is complete.  This can only be used with databases that have been
...
=== osmapidb.bulk.inserter.temp.file.dir

* Data Type: string
* Default Value: `/tmp`

Allows for customizing where the OSM API database bulk inserter stores temp files.
