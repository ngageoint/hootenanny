...
concept.  It is set as the default because other implementations, such as
hoot::HootServicesTranslatorClient, require custom installation procedures and have lengthy startup
times. Therefore, they are not available in the default Hootenanny installation.
...
The configuration option, 'language.translation.translator', controls which translator is used.
...
significant impact on the runtime performance of conflation when enabled.  The configuration
option, 'language.translation.translator', controls which translator is used.
