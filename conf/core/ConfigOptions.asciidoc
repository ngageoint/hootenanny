...
This is the maximum number of elements to write to an OSM API database in a changeset. This value is used when splitting a changeset into smaller pieces.

NOTE: This is different to `changeset.max.size` which is the maximum number of elements that the database can handle in a single changeset.
...
The maximum allowed element size of an OSM changeset that can be written to an OSM API database in a single changeset.
...
translation, setting this to try allows the detected language to override the specified source
languages.
