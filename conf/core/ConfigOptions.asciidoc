...
If the output destination is a SQL file and the writer application selected is "psql", the SQL
statements to update the record IDs will be written to the SQL output for later execution.  If the
output destination is a CSV file and the writer application selected is "pg_bulkload", the user is
