...
=== railway.type.review.threshold

* Data Type: double
* Default Value: `0.7`

Tag similarity threshold, in the range (0.0, 1.0], at or above which a railway feature is considered
to have tags similar enough with another rail feature to be considered a review if not already
considered a match.
...
List of tag key/value pairs that the `RailwaysCrossingMarker` will ignore when marking railways
which cross each other. Add to this list tags rail features possess where rail crossing is deemed
acceptable.
