...
=== api.db.reader.sort.by.id

* Data Type: bool
* Default Value: `false`

If true, elements read from the Hootenanny API database will be sorted first by element type and
then by element ID.
...
If this value is set to true, then all incoming element IDs are remapped into new IDs. This is
appropriate if any of the incoming IDs are non-positive.
...
If true, the OSM API database bulk writer will renumber element IDs, check for duplicated
