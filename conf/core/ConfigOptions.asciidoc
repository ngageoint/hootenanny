...
=== power.line.angle.sample.distance
...
=== power.line.rubber.sheet.minimum.ties
...
The search radius to use when conflating power lines.  Not used if
power.line.auto.calc.search.radius = true.  See `search.radius.default`.
...
adding/modifying a particular tag on all elements in a map. The number of keys in
...
adding/modifying a particular tag on all elements in a map.
