...
=== language.translation.ensemble.detectors
...
Language detectors used by the EnsembleLanguageDetector in order of entry.  Valid values are
'tika' and 'opennlp'.  If no detectors are specified, then language detection will attempt to use
as many detectors as needed to get a dection in a pre-determined order.  Separate values with a
';' and surround in quotes when passing in from the command line.

=== language.translation.ensemble.service.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/detect`

Web service endpoint used by the EnsembleLanguageDetector

=== language.translation.language.detectors

* Data Type: list
* Default Value:
** `hoot::EnsembleLanguageDetector`

Language detectors used by the ToEnglishTranslationVisitor.  Separate values with a ';' and surround
