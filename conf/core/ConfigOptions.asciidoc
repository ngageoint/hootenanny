...
converted data.  This is only applicable to the convert command.
...
* Data Type: list
* Default Value:
** `uuid` - Ignore UUID changes
** `source:datetime` - Ignore source:datetime changes
** `license`
** `source:imagery`
...
=== element.sorter.external.temp.format

* Data Type: string
* Default Value: `pbf`

The OSM format to use for writing sorted temp files during element external sorting.  Valid values
are 'osm' and 'pbf'.  Using 'pbf' will result in better performance for larger datasets and uses
the OpenStreetMap default resolution of 100 nanodegrees. 'osm' may be less performant and but allows
for using the higher Hootenanny internal resolution of .01 nanodegrees (same as used by the
...
=== hoot.services.auth.logout.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/auth/oauth1/logout`

Hootenanny Web Services endpoint for logging out of Hootenanny.

=== hoot.services.auth.request.token.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/auth/oauth1/request`

Hootenanny Web Services endpoint for retrieving Hootenanny request tokens.
...
=== hoot.services.auth.verify.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/auth/oauth1/verify`

Hootenanny Web Services endpoint for logging in a user.
...
A list of tag keys known to have been previously translated to the desired target language.  The
list ordering should correspond to that of language.translation.comparison.to.translate.tag.keys.
Separate keys with a ';' and surround in quotes when passing in from the command line.
...
=== language.translation.hoot.services.detectable.languages.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/detectable`

Service endpoint for retrieving detectable languages supported by HootServicesTranslatorClient

=== language.translation.hoot.services.detectors
...
When using 'HootServicesTranslatorClient' as the to English language translator, one or more
hoot-services Java class names implementing LanguageDetector.  Language detectors used
during a to English language translation in order of entry.  Valid values are 'TikaLanguageDetector'
and 'OpenNlpLanguageDetector'.  If no detectors are specified, then language translation will
attempt to use as many detectors as needed, in a pre-determined order, to get a positive detection .
Separate values with a ';' and surround in quotes when passing in from the command line.

=== language.translation.hoot.services.detectors.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/detectors`

Service endpoint for retrieving available detector implementations supported by
HootServicesTranslatorClient

=== language.translation.hoot.services.translatable.languages.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/translatable`

Service endpoint for retrieving translatable languages supported by HootServicesTranslatorClient

=== language.translation.hoot.services.translation.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/translate`

Web service translation endpoint used by the HootServicesTranslatorClient
...
When using 'HootServicesTranslatorClient' as the to English language translator, a hoot-services
Java class names implementing ToEnglishTranslator.  Valid values are 'JoshuaLanguageTranslator' and
'HootLanguageTranslator'.

=== language.translation.hoot.services.translators.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/translators`

Service endpoint for retrieving available translator implementations supported by
HootServicesTranslatorClient

=== language.translation.info.provider

* Data Type: string
* Default Value: `hoot::HootServicesTranslationInfoClient`

TranslationInfoProvider implementation to use for determining language information needed by to
English translation.  Not all to English translators will make use of this capability.

=== language.translation.max.cache.size

* Data Type: long
* Default Value: `10000`

Maximum result cache sized used for to English translation and transliteration.  Not honored by all
translators.  A cache size of -1 disables the translation cache.
...
When performing a to English translation , the translator will skip the translation of any tags
whose owning elements have a pre-translated English tag (key=<tag name>:en).

=== language.translation.skip.words.in.english.dictionary

* Data Type: bool
* Default Value: `true`

When performing a to English translation, the translator will skip the translation of any tags whose
value is in an English dictionary.
...
=== language.translation.to.translate.tag.keys

* Data Type: list
* Default Value:
** ``

A list of keys of tags whose values are to be translated to English.  The translations of these
tags are compared against the list of pre-translated tags in the option
language.translation.comparison.pretranslated.tag.keys (list sizes should be equal to each other).
Separate keys with a ';' and surround in quotes when passing in from the command line.

=== language.translation.translator

* Data Type: string
* Default Value: `hoot::DictionaryTranslator`

ToEnglishTranslator implementation to use for to English translation.  hoot::DictionaryTranslator,
the current default, is a simple lookup table primarily used for translation testing and proof of
concept.  Other implementations, such as hoot::HootServicesTranslatorClient, have better
translation capability but require custom server installation procedures.  If using
hoot::HootServicesTranslatorClient, the option 'hoot.services.authenticated.username' must be
populated with a valid value or translation web requests will fail.
...
* `hoot::HighwayMatchCreator` - matches roads using the non-greedy algorithm
* `hoot::NetworkMatchCreator` - matches roads using the Network algorithm.
* `hoot::PoiPolygonMatchCreator` - matches POIs with buildings or areas
* `hoot::ScriptMatchCreator,Area.js` - matches areas (non-building polygons marked as areas=yes).
* `hoot::ScriptMatchCreator,LinearWaterway.js` - matches linear rivers and streams
* `hoot::ScriptMatchCreator,PoiGeneric.js` - matches POIs using Generic Conflation
* `hoot::ScriptMatchCreator,PowerLine.js` - matches power lines using Generic Conflation
* `hoot::ScriptMatchCreator,Railway.js` - matches railways using Generic Conflation
...
TagKeyCountVisitor uses.  Separate keys with a ';' and surround entire list in quotes when passing
...
phone number comparisons.  You can expand the tag keys searched by populating this list. When
specifying the list from the command line, surround the entire value string in double quotes.
...
Optional geographical region code used when comparing phone numbers.  Specifying a region code, if
known for the input data, may increase phone number matching accuracy.  See:
...
address.  This can have significant impact on the runtime performance of conflation when enabled
...
=== poi.polygon.translate.names.to.english
...
=== poi.polygon.translate.types.to.english
...
list items with a semicolon and surround the entire list in quotes.
...
Flat file containing street types and their abbreviations.  Used in various parts of the
application.
...
delimit multiple key/value pairs with ';' and surround the entire list in quotes.
...
semicolon from the command line and surround the entire list in quotes.
