...
The direction that the translation script should translate. `toogr` will translate from to OGR.
`toosm` will translate from to OSM. This is used with the convert command, TranslationOp, or
TranslationVisitor.

TODO: make this default to empty
...
The script used by the TranslationVisitor for schema translation.
