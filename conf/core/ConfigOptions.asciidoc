...
This changes the behavior for reads from API databases over a bounds using `convert.bounds` to
...
delete statements for reference features falling outside of `convert.bounds`.
...
The map operations used by replacement changeset generation.
...
=== collection.relation.type.threshold
...
If specified, supporting readers will limit data read from the source to only features that
intersect a given rectangular bounding box or closed polygon. Not all supporting readers allow for
the closed polygon in addition to the rectangular bounding box. The format can be either:
"minx,miny,maxx,maxy" ("min_lon,min_lat,max_lon,max_lat") for a rectangular bounds or
"x1,y1;x2,y2;x3,y3;..." ("lon1,lat1;lon2,lat2;lon3,lat3...") for a closed polygon. The coordinates
are specified in the projection of the input data source.  This option can be used with both the
convert and conflate commands. See the command documentation for more details.

Example Usage:

----
hoot convert -D convert.bounds="106.851,-6.160,107.052,-5.913" input.osm output.osm
----

=== convert.bounds.hoot.api.database

* Data Type: string
* Default Value: ``

Same as `convert.bounds` but the resultant bounds filtering is only applied to Hootenanny API
database data sources when used with the convert and conflate commands.  This setting takes
precendence over the `convert.bounds` setting for Hootenanny API database data sources only.

=== convert.bounds.keep.entire.features.crossing.bounds
...
=== convert.bounds.keep.immediately.connected.ways.outside.bounds

* Data Type: bool
* Default Value: `false`

If enabled, bounded queries will also return ways that are outside of `convert.bounds` but
immediately connected to ways crossing or within the bounds. Can be used in conjunction with
`convert.bounds.tag.immediately.connected.out.of.bounds.ways`. Not supported by all readers.

=== convert.bounds.keep.only.features.inside.bounds
...
=== convert.bounds.osm.api.database

* Data Type: string
* Default Value: ``

Same as `convert.bounds` but the resultant bounds filtering is only applied to OSM API database
data sources when used with the convert and conflate commands.  This setting takes precendence over
the `convert.bounds` setting for OSM API database data sources only.

=== convert.bounds.remove.missing.elements

* Data Type: bool
* Default Value: `true`

If enabled, any call to the `convert` command with a bounds specified by `convert.bounds` will
remove missing references to missing elements as a result of map cropping.

=== convert.bounds.tag.immediately.connected.out.of.bounds.ways

* Data Type: bool
* Default Value: `false`

If enabled, all ways that are outside of the bounds specified by `convert.bounds` when
...
`convert.bounds.keep.immediately.connected.ways.outside.bounds`. Not supported by all readers.
...
Bounds used by the map cropper when cropping a map. See the `convert.bounds` option for format
detail.
...
The bounding box used by InBoundsCriterion. See the `convert.bounds` option for format detail.
...
If true, then the coordinates specified by `convert.bounds` are assumed to be in the WGS84
coordinate system.
...
* `hoot::ReplacementSnappedWayJoiner` - has way joining feature specific to Cut and Replace
