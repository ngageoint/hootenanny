...
of the conflation pre-operations.
...
This allows for overriding the match/review distances for POI to POI Conflation defined in the file
specified by +poi.search.radii.config+. Its JSON format matches that in the file referenced by
+poi.search.radii.config+.
...
Points to a JSON file defining the default search radii distance configuration for POI to POI
Conflation. Use of this file is overridden when +poi.search.radii+ is specified.
...
Determines which type of features are rubbersheeted.
