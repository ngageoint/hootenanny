...
This changes the behavior for reads from API databases over a bounds using `convert.bounding.box` to
...
=== changeset.replacement.cache.input.maps

* Data Type: bool
* Default Value: `false`

If enabled, changeset replacement derivation performed by `changeset-derive-replacement` will load
each entire input once and calculate the changeset output based on cropped versions of the loaded
input rather than reading the input data separately for each geometry type processing pass. Only
enable this if the inputs consist of files or database layers that will fit easily in memory before
cropping them to the replacement bounds. TODO: change this to be used for files only and default to
true
