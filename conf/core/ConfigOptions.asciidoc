...
List of tag keys with values identifying how many tracks a particular rail has. If any of the tags
are found in a rail from the second input, railway matching will attempt a one to many match from
input 2 to input 1 with matches up to the number of tracks specified in the tag value (arbitarily,
the first tag found from the key list is used). Only tag merging of selected tags (see
