...
If false, outputting to an OSM API database will write the SQL file to be executed only without
executing the SQL against the database.  Use this setting in conjuction with the
osmapidb.bulk.writer.sql.output.file.copy.location setting to view SQL output before applying it to
a database.
...
Valid values are "offline" and "online".  Offline mode does not guarantee element ID uniqueness
but will run faster than online mode.  Offline mode is meant to be used against offline databases
(ones with no other writers present).  Online mode guarantees element ID uniqueness, is safe to run
against a live database (other writers present), but will run more slowly than offline mode.

=== osmapidb.bulk.writer.sql.output.file.copy.location

* Data Type: string
* Default Value: ``

If populated, the generated SQL file whose statements are executed against the target database as
the result of a hoot convert command with an OSM API database target is copied to the specified
...
This is required when using either the OsmApiDbAwareHootApiDbReader, OsmApiDbAwareHootApiDbWriter,
or PostgresqlDumpfileWriter with the postgresql.dumpfile.writer.auto.calc.ids=true setting.  It
forces database reading/writing use the specified OSM API database as master for determining the
