...
-2, ...). The `PositiveIdGenerator` increments the IDs (e.g. 1, 2, 3, ...).
...
`OsmValidator::CORE_TEST_CLASSES` member variable. Last updated version: 18118. See also:
`hoot.validators`.
...
and is included in both datasets to not be duplicated when read into Hootenanny.
...
When enabled, the Unconnected Way Snapper will attempt to maximize the number of reference
(status=1) way nodes kept during snapping.
...
must inherit from `ConflatableElementCriterion`. An example is: `HighwayCriterion`. This
option combines with `snap.unconnected.ways.snap.to.way.statuses` to determine which ways are
eligible to be snapped to and overrides `snap.unconnected.ways.snap.criteria`.
...
then feature status is ignored for ways or way nodes being snapped to. This option combines with
`snap.unconnected.ways.snap.to.way.criteria` to determine which ways are eligible to be snapped to.
...
must inherit from `ConflatableElementCriterion`. An example is: `HighwayCriterion`. This
option combines with `snap.unconnected.ways.snap.way.statuses` to determine which ways are eligible
for snapping and overrides `snap.unconnected.ways.snap.criteria`.
...
status is ignored for ways or way nodes being snapped. This option combines with
`snap.unconnected.ways.snap.way.criteria` to determine which ways are eligible for snapping.
...
The default ancestor to use when using the TagAncestorDifferencer class for comparing tags.
...
The default category to use when using the TagCategoryDifferencer class for comparing tags.
Primarily, this is useful within the node.js interface.
...
Path to the translation script when using the tag differencer, `TranslatedTagDifferencer`.
