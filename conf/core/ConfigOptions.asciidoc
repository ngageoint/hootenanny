...
completely and only the snap criterions will influence the snapping.
...
Optional criterion class list used to filter both the types of ways being snapped (sources) and
the types of ways being snapped to (targets). The classes specified must inherit from
`ConflatableElementCriterion`. An example is: `hoot::HighwayCriterion`. The default  value of
`hoot::WayCriterion` allows for snapping any type of linear feature. This option is overridden by
the options: `snap.unconnected.ways.snap.way.criteria` and/or
`snap.unconnected.ways.snap.to.way.criteria` to allow for more granularity in selecting the types
to snap.
...
Optional criterion class list used to filter the types of ways being snapped to (targets). The
classes specified must inherit from `ConflatableElementCriterion`. An example is:
`hoot::HighwayCriterion`. This option overrides `snap.unconnected.ways.snap.criteria`.
...
Optional criterion class list used to filter the types of ways being snapped (sources). The classes
specified must inherit from `ConflatableElementCriterion`. An example is: `hoot::HighwayCriterion`.
This option overrides `snap.unconnected.ways.snap.criteria`.
...
Name similarity threshold, in the range (0.0,1.0], at or above which a linear waterway feature is
...
Tag similarity threshold, in the range (0.0,1.0], at or above which a waterway element is considered
