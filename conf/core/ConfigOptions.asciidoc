...
=== changeset.allow.deleting.from.features

* Data Type: bool
* Default Value: `true`

If true, changesets derived can issue delete statements for the 'from' dataset (first dataset
passed to the changeset deriver).  If false, no delete statements will be issued for the 'from'
dataset.

=== changeset.compare.status

* Data Type: bool
* Default Value: `true`

If true, changeset elements are considered different if they have different statuses.  If false,
then status does not matter during element comparison.  Setting this to true may result in many
additional changeset modify statements when Hootenanny changes the status of elements to
'conflated'.
...
=== reader.conflate.use.data.source.ids

* Data Type: bool
* Default Value: `false`

Determines whether the reader used by the conflate command will use the element ID's specified
by the input datasets (true) or use internal ID management for the inputs (false).
