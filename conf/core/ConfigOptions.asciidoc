...
If true, instances where multiple POIs were matched to a single polygon will result in all of the
POIs being merged into the polygon.  If false, then each POI will generate a review against the
polygon instead.
...
If true, POI to Polygon conflation will treat only the POI that is closest to a polygon as a
distance match (multiple closest distance matches are possible in the case of distance ties) and
not match any other POIs, even if they are within the match distance threshold.  If false, then all
POIs within the match distance threshold of a polygon are treated as distance matches.
