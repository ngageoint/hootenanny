...
features that conflation is unable to handle. Keep the list alphabetized by key, then value (we may
want to eventually move it to its own config file).
...
duplicates and not copy them. The higher the value, the more two nodes are considered duplicates.
