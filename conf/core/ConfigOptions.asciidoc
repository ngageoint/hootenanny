...
=== tag.values.max.tag.values.per.tag.key

* Data Type: int
* Default Value: `-1`

The maximum number of unique tag values per tag key with values returned for by the
tag-values command.  A value of -1 indicates there is no limit.
