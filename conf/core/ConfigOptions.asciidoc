...
=== osmapidb.bulk.writer.reserve.record.ids

* Data Type: bool
* Default Value: `true`

If true, the OSM API database writer will reserve the range of record IDs parsed from the input
data.  The output destination must be an OSM API database or this setting will be ignored.  This
should always be set to true if you are writing directly to an online database (other writers are
present).  If this option is set to false, the user is responsible for all record ID management for
