...
translating with the convert command, and the inputs/outputs are not OGR formats, the convert
command will use the `toosm` direction and log a message indicating that no direction was specified.
The TranslationOp and TranslationVisitor used outside of the convert command will return an error
