...
=== generic.point.match.threshold

* Data Type: double
* Default Value: `${conflate.match.threshold.default}`

The threshold at which a match is called a match for generic points.

See also:

 * _Estimate Pairwise Relationships_, <<hootalgo>>

=== generic.point.miss.threshold

* Data Type: double
* Default Value: `${conflate.miss.threshold.default}`

The threshold at which a miss is called a miss for generic points.

See also:

 * _Estimate Pairwise Relationships_, <<hootalgo>>

=== generic.point.review.threshold

* Data Type: double
* Default Value: `${conflate.review.threshold.default}`

The threshold at which a review is called a review for generic points. See
`conflate.review.threshold.default`.
...
=== generic.point.polygon.match.threshold

* Data Type: double
* Default Value: `${conflate.match.threshold.default}`

The threshold at which a match is called a match for generic points conflated with polygons.

See also:

 * _Estimate Pairwise Relationships_, <<hootalgo>>

=== generic.point.polygon.miss.threshold

* Data Type: double
* Default Value: `${conflate.miss.threshold.default}`

The threshold at which a miss is called a miss for generic points conflated with polygons.

See also:

 * _Estimate Pairwise Relationships_, <<hootalgo>>

=== generic.point.polygon.review.threshold

* Data Type: double
* Default Value: `${conflate.review.threshold.default}`

The threshold at which a review is called a review for generic points conflated with polygons. See
`conflate.review.threshold.default`.
...
* Data Type: list
* Default Value:
** `${HOOT_HOME}/bin/josm.jar;${HOOT_HOME}/bin/hoot-josm.jar`

A list of JAR files to place on the JAVA classpath when Hootenanny C++ code interacts with Java
code using JNI.
...
JVM -Xms setting to use with JNI to configure the initial amount of memory available to the JVM.
...
JVM -Xmx setting to use with JNI to configure the maximum amount of memory available to the JVM.
...
* Data Type: string
* Default Value: `hoot/services/josm/JosmMapValidator`

File path relative to Hootenanny root directory without file extension to Java class to use as the
JOSM Java map valdidator/cleaner implementation.
...
A list of JOSM validator Java class names without namespace prefixes to explicitly exclude use of
during validation or cleaning using JOSM. Overrides values set in josm.validators.include.
...
A list of JOSM validator Java class names without namespace prefixes to explicitly include use of
during validation or cleaning using JOSM. Overridden by values set in josm.validators.exclude.
...
A list of JOSM validators known to require the user certificate specified in josm.certificate.path.
These validators will be skipped if no user certificate is specified to avoid seeing unnecessary
error message from JOSM when they cannot be properly run without the certificate. Clearly, this list
may become out of sync with JOSM over time without maintenance.
...
=== map.cleaner.josm.add.detail.tags

* Data Type: bool
* Default Value: `false`

If true, cleaning detail tags will be added to map outputs when cleaning maps with JOSM.
...
The search radius to use when conflating generic points.  See `search.radius.default`.
...
The search radius to use when conflating generic points with generic polygons.  See
`search.radius.default`.
