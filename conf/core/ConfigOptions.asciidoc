...
=== poi.polygon.translate.tag.values.to.english

* Data Type: bool
* Default Value: `false`

If true, Hootenanny will attempt to translate to English the value of any tag having a key in its
schema or one that is part of an address.  This can have significant impact on the runtime
performance of conflation when enabled.  The configuration option,
