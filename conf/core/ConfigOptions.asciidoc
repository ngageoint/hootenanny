...
=== compare.include.all.elements

* Data Type: bool
* Default Value: `false`

The `hoot compare` command saster scoring options use the `HighwayCriterion` to filter elements to
render for comparison. Setting this option to `true` will not filter any elements, all elements will
be rendered and the final render will be compared. Good for comparing building rasters.
...
=== join.partial.polygons

* Data Type: bool
* Default Value: `false`

When exporting to OGR formats, if a polygon member of a multipolygon is composed of multiple different non-closed ways,
consolidate them into a single closed way. This can cause problems with cropped datasets, because they turn polygons
into non-circular ways, and forcing them to be circular leads to erroneous closures.
...
new file/layer. Schema templates to append to can be found in $HOOT_HOME/translations/templates/.
