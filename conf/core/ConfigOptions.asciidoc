...
If true, Differential Conflation will snap unconnected secondary road endpoint nodes to the nearest
reference road.
...
Maximum distance used by Differential Conflation, in meters, allowed between an unconnected
secondary road endpoint node and neighboring reference road node for the secondary road endpoint
node to be snapped directly to the existing reference road node instead of adding the secondary
road endpoint node as a new road node on the reference road.
...
Maximum distance used by Differential Conflation, in meters, allowed between an unconnected
secondary road endpoint node and neighboring reference road for the secondary road endpoint node to
be snapped to the closest section of the reference road.
...
If true, when Differential Conflation snaps secondary road endpoint nodes to reference roads, it
will attempt to reuse existing road nodes on the reference road being snapped to instead of adding
the secondary road endpoint node to the reference road.
...
If specified, when Differential Conflation snaps secondary road endpoint nodes to reference roads,
it will tag them with the specified tag key.
...
=== hootapi.db.writer.job.id

* Data Type: string
* Default Value: ``

This option allows the Hootenanny Web Services to pass in the ID of the currently executing command
job.  Given that information, any Hootenanny API database writer invoked by the command will write
...
nested criteria, use the Hootenanny Javascript interface instead.
