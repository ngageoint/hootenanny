...
* Data Type: string
* Default Value: `hoot::HootServicesLanguageDetectorMockClient`

LanguageDetector implementation to use for detecting source languages.  The default value,
hoot::HootServicesLanguageDetectorMockClient, is used for demonstration purposes only.  If using
hoot::HootServicesLanguageDetectorClient, which performs useful detection, a translation server
must be set up and valid OAuth credentials used.
...
=== language.hoot.services.detectable.languages.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/detectable`

Service endpoint for retrieving detectable languages supported by Hootenanny Web Services language
detection/translation (when using HootServicesTranslatorClient or HootServicesLanguageDetectorClient).
...
class names implementing LanguageDetector.  Language detectors are used in order of entry.  Valid
values are 'TikaLanguageDetector' and 'OpenNlpLanguageDetector'.  If no detectors are specified,
then language translation will attempt to use as many detectors as needed, in a pre-determined
order, to get a positive detection. Separate values with a ';' and surround in quotes when passing
in from the command line.

=== language.hoot.services.detection.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/detect`

Service language detection endpoint used by HootServicesTranslatorClient and
HootServicesLanguageDetectorClient

=== language.hoot.services.detectors.endpoint

* Data Type: string
* Default Value: `http://localhost:8080/hoot-services/language/toEnglishTranslation/detectors`

Service endpoint for retrieving available language detector implementations supported by
HootServicesTranslatorClient and HootServicesLanguageDetectorClient

=== language.hoot.services.translatable.languages.endpoint
...
=== language.hoot.services.translation.endpoint
...
=== language.hoot.services.translators.endpoint
...
* Data Type: string
* Default Value: `hoot::HootServicesTranslationInfoMockClient`

TranslationInfoProvider implementation to use for determining language information needed by to
English translation.   The default value,
hoot::HootServicesTranslationInfoMockClient, is used for demonstration purposes only.  If using
hoot::HootServicesTranslationInfoClient, a translation server must be set up and valid OAuth
credentials used. Not all to English translators will make use of this capability.
...
A list of keys of tags whose values are to either be translated to English by the
ToEnglishTranslationVisitor or just have their source languages detected by the the
...
list ordering should correspond to that of language..tag.keys.  Separate keys with a ';' and
surround in quotes when passing in from the command line.
...
the current default value, is a simple lookup table primarily used for translation testing and
proof of concept.  hoot::HootServicesTranslatorClient, which performs more useful translation,
requires a translation server be set up and valid OAuth credentials used.
