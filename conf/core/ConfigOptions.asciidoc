...
=== changeset.replacement.cache.input.file.maps

* Data Type: bool
* Default Value: `true`

If enabled, the changeset replacement derivation performed by `changeset-derive-replacement` will
load each entire file input once and calculate the changeset output based on cropped versions of the
loaded input rather than reading the input data separately for each geometry type processing pass.
This is advantageous for file inputs only. Only enable this if the file inputs to
`changeset-derive-replacement` consist of data that will fit easily into available memory.
