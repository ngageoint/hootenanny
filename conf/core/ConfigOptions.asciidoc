...
Specifies a list of directories for the TranslationServer to look for translation scripts. The TranslationServer.js
script will try to load a translationServerConfig.js file from each directory specified.
To change this at runtime, add a line to the $HOOT_HOME/conf/hoot.json with the list of directories.
E.g.  "convert.translation.dir.list":"translations;translations-local;papaSmurf",
NOTE: These directories are expected to be in the Hootenanny root directory ($HOOT_HOME)
