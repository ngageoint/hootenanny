...
=== osmapidb.bulk.writer.app

* Data Type: string
* Default Value: `psql`

Valid values are "psql" or "pg_bulkload".  This option is ignored if the output destination is a
SQL file or set of CSV files.  The pg_bulkload writer application provides options for increased
database write performance, some of which are not safe for using against a live database (other
writers exist).  To use the pg_bulkload writer application the pg_bulkload extension must be
installed on the destination database.  See the user documentation for details.

=== osmapidb.bulk.writer.disable.constraints

* Data Type: bool
* Default Value: `false`

If true, OSM API database writes will bypass Postgres database constraint checks, which are a
safety feature to ensure data integrity.  This will increase database writing performance but will
possibly require extra database maintenance after the data is loaded if the data has integrity
issues.  The option "osmapidb.bulk.writer.app" must be set to "pg_bulkload" to enable this option.
IMPORTANT: Hootenanny does basic constraint checking for OSM data during loading, but this option
should still only be enabled when there is high confidence in the integrity of the data being
written.

=== osmapidb.bulk.writer.disable.write.ahead.logging

* Data Type: bool
* Default Value: `false`

If true, OSM API database writes will bypass Postgres Write-Ahead Logging (WAL), which is a
safety feature to ensure data integrity.  This will increase database writing performance but will
require additional database maintenance after the data is loaded.  The option
"osmapidb.bulk.writer.app" must be set to "pg_bulkload" to enable this option.  IMPORTANT:  This
should only be enabled the database has previously been taken offline (you are the only writer).

=== osmapidb.bulk.writer.file.output.buffer.max.line.size

* Data Type: long
* Default Value: `1000000`

Maximum number of lines to keep in memory when serializing elements to a file output, where each
line represents a separate SQL record.  If you have enough memory available and the dataset you are
writing is larger than the value of this setting, you may be able to increase the input file parsing
performance by raising this value.  This setting has no impact on the performance of writing records
to a database, only on the input data parsing done beforehand.
...
file lines written.  This setting will have a negative impact on performance if set to a very low
value.

=== osmapidb.bulk.writer.multithreaded

* Data Type: bool
* Default Value: `false`

If true, OSM API database writes will be performed with multiple threads.  This will increase
database writing performance but may degrade database performance for other database users.  The
option "osmapidb.bulk.writer.app" must be set to "pg_bulkload" to enable this option.
...
This option is not needed and will be ignored if writing OSM data to a SQL file or set of CSV
files that will be applied/written to an OSM API database at a later time.  If this option is
populated, any file data files generated as a result of executing the hoot convert command
with an OSM API database target are copied to the specified location.  If "osmapidb.bulk.writer.app"
is set to "psql", this option should be set to a full file path to a SQL file (.sql).  If
"osmapidb.bulk.writer.app" is set to "pg_bulkload", this should be set to a directory path where
a set of CSV data files will be written.

=== osmapidb.bulk.writer.pgbulkload.bad.records.log.path

* Data Type: string
* Default Value: ``

Optional path to the log the pg_bulkload utility writes any bad records it encounters to.  This
option is ignored if the option "osmapidb.bulk.writer.app" is set to anything other than
"pg_bulkload".

=== osmapidb.bulk.writer.pgbulkload.log.path

* Data Type: string
* Default Value: ``

Optional path to the log the pg_bulkload utility writes to.  This option is ignored if the option
"osmapidb.bulk.writer.app" is set to anything other than "pg_bulkload".
...
written to the SQL output along with the other record data.  If the output destination is a
CSV file, the SQL statements to update the record IDs will be written to a separate SQL file in the
same location as the CSV files.
...
First record ID to assign to written nodes when writing to an OSM API database or SQL file.  Use
this when working with an offline database and know the ID range you want to assign to node records.
This option is ignored if "osmapidb.bulk.writer.reserve.record.ids.before.writing.data" is set to
true.
...
First record ID to assign to written ways when writing to an OSM API database or SQL file.  Use
this when working with an offline database and know the ID range you want to assign to way records.
This option is ignored if "osmapidb.bulk.writer.reserve.record.ids.before.writing.data" is set to
true.
...
First record ID to assign to written relations when writing to an OSM API database or SQL file.  Use
this when working with an offline database and know the ID range you want to assign to relation
records.  This option is ignored if "osmapidb.bulk.writer.reserve.record.ids.before.writing.data"
is set to true.
