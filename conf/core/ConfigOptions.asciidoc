...
f true, any buildings in the secondary layer will be automatically reviewed against potentially
...
=== libaddressinput.address.rules.file

* Data Type: string
* Default Value: `${HOOT_HOME}/conf/core/poi-polygon/countryinfo.txt`

JSON rules file used by libaddressinput for address normalization/validation.
...
=== poi.polygon.address.country.code

* Data Type: string
* Default Value: `US`

ISO 3166-2 country code used when comparing addresses. TODO: is this absolutely needed, and if so,
can it be combined with the phone number region code?
