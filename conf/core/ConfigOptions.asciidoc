...
hoot::HootServicesLanguageDetectorClient, the option 'hoot.services.authenticated.username' must be
...
If true, source languages detected for tags will be written in new tags to output.  If false, then
only a summary of the source languages found will be printed.
...
tag already exists on the associated element and attempt to detect the source language of tags
anyway.
