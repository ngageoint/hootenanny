...
=== snap.unconnected.ways.way.discretization.spacing

* Data Type: double
* Default Value: `0.1`

The spacing granularity to which a unconnected way node is snapped back to a way with the
Unconnected Way Snapper.

=== snap.unconnected.ways.way.node.reuse.tolerance
...
the existing way node instead of adding the way endpoint node as a new way node on the way.  A
value of 0.0 or less indicates no node reuse will occur.

=== snap.unconnected.ways.reuse.way.nodes
...
section of the neighboring way.  A value of 0.0 or less indicates no snapping will occur.

=== snap.unconnected.ways.tag.key

* Data Type: string
* Default Value: ``

If specified, when the Unconnected Way Snapper snaps way endpoint nodes to neighboring ways, it
will tag them with the specified tag key.
