...
=== element.counter.chain.element.criteria

* Data Type: bool
* Default Value: `false`

If set to true and multiple criterion are specified to the `count` command or passed directly into
`ElementCounter`, an element must satisfy each criterion in order to pass the counting filter.  If
set tofalse, then only one of the specified criteria must be satisified in order to pass the
counting filter.
