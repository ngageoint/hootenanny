...
The user ID used when writing to an OSM API database used by changeset writers and the
...
=== osmapidb.bulk.writer.file.output.element.status.interval

* Data Type: long
* Default Value: `1000000`

Count at which the SQL file output status of each element type should be logged by the
when bulk writing to an OSM API database.  e.g. With a value of 100, you will see a element load
status update message logged for every 100 elements parsed.
...
but will run faster than online mode and is meant to be used against offline databases (ones with
no writers present).  Online mode guarantees element ID uniqueness, is safe to run against a live
database, and will run more slowly than offline mode.
...
Count at which the load status of each element type should be logged by the PostgresqlDumpfileWriter.
e.g. With a value of 100000, you will see a element load status update message for every 100000
elements parsed.
