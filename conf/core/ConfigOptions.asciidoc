...
=== osmapidb.bulk.writer.bypass.write.ahead.logging

* Data Type: bool
* Default Value: `false`

If true, the OSM API database writes will bypass Postgres Write-Ahead Logging (WAL), which is a
safety feature to ensure data integrity.  This should only be enabled when there is high confidence
in the integrity of the data being written.
...
If true, the OSM API database writes will be performed with multiple threads.
