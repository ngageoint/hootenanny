...
If true, the OSM API database writes will bypass Postgres Write-Ahead Logging (WAL), which is a
safety feature to ensure data integrity.  IMPORTANT: This should only be enabled when there is
high confidence in the integrity of the data being written and the database has been taken offline
...
If true, the OSM API database writes will be performed with multiple threads.  IMPORTANT: This
should only be enabled when the database has previously been taken offline (you are the only
writer).
