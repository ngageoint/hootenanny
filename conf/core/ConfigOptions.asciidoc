...
possibly require extra database maintenance after the data is loaded.  The option
'osmapidb.bulk.writer.mode' must be set to 'offline' to enable this setting.  IMPORTANT: Hootenanny
does basic constraint checking for OSM data during load, but this should still only be enabled when
there is high confidence in the integrity of the data being written.
...
'osmapidb.bulk.writer.mode' must be set to 'offline' to enable this setting.  IMPORTANT:  This
...
Maximum number of lines to keep in memory when serializing the OsmApiDbBulkWriter's SQL output.
For the most part, each line represents an element or an element tag.

=== osmapidb.bulk.writer.execute.sql

* Data Type: bool
* Default Value: `true`

If false, outputting to an OSM API database will write the intermediate step, temporary SQL or CSV
files to be executed only without actually writing the records to the database.  Use this setting
in conjuction with the osmapidb.bulk.writer.output.file.copy.locatio setting to view SQL or CSV
...
Count at which the SQL file output status of each element type should be logged when bulk writing
the SQL file that is to be applied to an OSM API database.  e.g. With a value of 100, you will see
a load status update message logged for every 100 SQL lines parsed.

=== osmapidb.bulk.writer.mode

* Data Type: string
* Default Value: `online`

Valid values are "online" or "offline".  Online mode is safe for use against a live database
(multiple writers).  Offline mode is unsafe for use against a live database but will have better
writing performance.  Offline mode requires the the pg_bulkload extension be installed on the
database being written to, will not roll back all database changes in the event of a failure, and
has additional maintenance requirements.  Some osmapidb.* options are only available in offline
mode.  See the User Documentation for more details.
...
If true, the OSM API database writes will be performed with multiple threads.  This will increase
database writing performance but may degrade database performance for other users.  The option
'osmapidb.bulk.writer.mode' must be set to 'offline' to enable this setting.

=== osmapidb.bulk.writer.offline.log.path

* Data Type: string
* Default Value: ``

Optional path to the log the pg_bulkload utility writes to when writing OSM API database data in
offline mode.

=== osmapidb.bulk.writer.offline.bad.records.log.path

* Data Type: string
* Default Value: ``

Optional path to the log the pg_bulkload utility writes any bad records it encounters while writing
OSM API database data in offline mode.

=== osmapidb.bulk.writer.output.file.copy.location

* Data Type: string
* Default Value: ``

If populated, any generated temporary files generated as a result of executing the hoot convert
command with an OSM API database target are copied to the specified location.  If
osmapidb.bulk.writer.mode is set to "online", this should be set to an full file path to a SQL file
(.sql).  If osmapidb.bulk.writer.mode is set to "offline", this should be set to a directory in
which a set of CSV files will be written.  Use this if you wish to manually apply the changes to
your database.
