...
StringDistance implementation used to score translated tag values for comparison
...
=== language.translation.language.detectors

* Data Type: list
* Default Value:
** `hoot::TikaLanguageDetector`
** `hoot::OpenNlpLanguageDetector`

Language detectors used by the ToEnglishTranslationVisitor.  Separate keys with a ';' and surround
in quotes when passing in from the command line.

=== language.translation.opennlp.library

* Data Type: string
* Default Value: `opennlp-tools-1.8.3.jar`

Library used by OpenNlp language detection.  The file should exist in
conf/core/language-translation/open-nlp.  This configuration option will be obsolete once language
detection moves to hoot-services.

=== language.translation.opennlp.model

* Data Type: string
* Default Value: `langdetect-183.bin`

Model used by OpenNLP language detection.  The file should exist in
conf/core/language-translation/open-nlp.  This configuration option will be obsolete once language
detection moves to hoot-services.
...
=== language.translation.service.mappings.file

* Data Type: string
* Default Value: `conf/core/language-translation/languageServicePortMappings`

Path to a file which contains information about available to English language translation services.
This configuration option will be obsolete if language translation moves to hoot-services.
...
When performing to English translation with ToEnglishTranslationVisitor, the translator will skip
the translation of any tags whose owning elements have a pre-translated English tag
(key=<tag name>:en).
...
When performing to English translation with ToEnglishTranslationVisitor or
ToEnglishTranslationComparisonVisitor, the translator will skip the translation of any tags whose
value is in an English dictionary.

=== language.translation.supported.languages.file

* Data Type: string
* Default Value: `conf/core/language-translation/supportedToEnglishTranslationLanguages`

Path to a file which contains information about supported languages for to English translation

=== language.translation.tika.library

* Data Type: string
* Default Value: `tika-app-1.18.jar`

Library used by Tika language detection.  The file should exist in
conf/core/language-translation/tika.  This configuration option will be obsolete once language
detection moves to hoot-services.
...
A list of keys of tags whose values are to be translated to English when using the
ToEnglishTranslationVisitor or the ToEnglishTranslationComparisonVisitor.  When using the
ToEnglishTranslationComparisonVisitor, the translations of these tags are compared against
the list of pre-translated tags in the option language.translation.comparison.pretranslated.tag.keys
(list sizes should be equal to each other).  Separate keys with a ';' and surround in quotes when
...
ISO-639-1 language codes for the source languages of the to English translation done by
ToEnglishTranslationVisitor and ToEnglishTranslationComparisonVisitor
