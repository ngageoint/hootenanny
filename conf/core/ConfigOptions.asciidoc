...
This should only be enabled when there is high confidence in the integrity of the data being
written.
...
safety feature to ensure data integrity.  This will increase database writing performance.
The option 'osmapidb.bulk.writer.mode' must be set to 'offline' to enable this setting.  IMPORTANT:
This should only be enabled when there is high confidence in the integrity of the data being written
and the database has been taken offline (you are the only writer).
...
If populated, any generated SQL or CSV files generated as a result of a hoot convert command with
an OSM API database target are copied to the specified location. For debugging purposes or if you
wish to manually apply the changes to your database.
