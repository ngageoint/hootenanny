...
=== conflate.preserve.reference.element.id.when.merging.features

* Data Type: bool
* Default Value: `false`

If true, the element ID of reference features will preserved when merging features during
conflation.  If false, the merged feature will be given a new, unique element ID.  This is useful
when writing conflated data output back to a reference data store where modifying existing features
is preferred over replacing them.  Not honored by all conflation mergers.
...
Sets the status on data being read.  Not honored by all readers.
