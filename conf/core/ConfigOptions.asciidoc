...
=== element.sorter.in.memory

* Data Type: bool
* Default Value: `false``

If true, all element sorting will occur in memory.  If false, temporary disk storage will be used
for storage if the number of elements to be sorted is larger than
element.sorter.external.merge.max.element.size.
...
When the element.sorter.in.memory is set to false, the maximum number of elements to be written to
a single file during sorting.
