...
If enabled, bounded queries will also return ways that are outside of `bounds` but
...
the commands supporting the `bounds` option only and kept as a separate option.
...
delete statements for reference features falling outside of `bounds`.
...
the first element's ID will be updated.
