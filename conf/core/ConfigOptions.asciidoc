...
false, then only one of the specified criteria must be met in order to modify an the attributes of
an element.
...
Optional criteria classes to filter the types of ways being snapped to. You may add multiple
criteria and separate them with a semicolon surrounded by quotes. An example would be:
"hoot::HighwayCriterion;hoot::LinearWaterWayCriterion" to snap road and river ways.  If no criteria
are specified, then all ways are considered as targets for way snapping.

=== snap.unconnected.ways.snap.to.way.node.criteria

* Data Type: list
* Default Value:
** `hoot::WayNodeCriterion`

Optional criteria classes to filter the types of way nodes being snapped to. You may add multiple
criteria and separate them with a semicolon surrounded by quotes.  An example would be:
"hoot::HighwayNodeCriterion".  If no criteria are specified, then all way nodes are considered as
targets for way node snapping.  This option is only used if snap.unconnected.ways.reuse.way.nodes
is enabled.
...
Statuses of ways or way nodes being snapped to.  Valid values are 'Input1', 'Input2', or
'Conflated'. Specify 'Input1' to snap secondary ways to reference ways or 'Input2' to snap reference
ways to secondary ways. You may add multiple statuses and separate them with a semicolon surrounded
by quotes. For example, to snap to both reference and conflated ways use: 'Input1;Conflated'.
...
Optional criteria classes to filter the types of ways being snapped. You may add multiple
criteria and separate them with a semicolon surrounded by quotes. An example would be:
"hoot::HighwayCriterion;hoot::LinearWaterwayCriterion.  If no criterion is specified, then all ways
are considered as sources for way snapping.
...
ways. You may add multiple statuses and separate them with a semicolon surrounded
by quotes. For example, to snap both reference and conflated ways use: 'Input1;Conflated'.
