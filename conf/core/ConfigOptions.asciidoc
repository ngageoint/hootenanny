...
=== language.translation.comparison.to.translate.tag.keys

* Data Type: list
* Default Value:
** ``

A list of tag keys for tag translation comparison against pre-translated tags.  The list ordering
should correspond to that of language.translation.comparison.pretranslated.tag.keys.  Separate
keys with a ';' and surround in quotes when passing in from the command line.
...
ISO-639 language code for the source language of the translation
...
ISO-639 language code for the source language of the translation.  Currently only English (en) is
supported.
