...
=== way.joiner.advanced.keep.feature.status.override

* Data Type: string
* Default Value: ``

Overrides the default behavior of `WayJoinerAdvanced` for determining which set of feature tags
are kept when joining ways. Valid values are 'unknown1', 'unknown2', or 'conflated'. If 'unknown1'
or 'unknown2' is specified, neither feature in the pair of features being joined has that status,
and at least one of them has a conflated status, then 'conflated' will be used instead. This option
will always override the behavior of `tag.merger.default` when joining ways.
