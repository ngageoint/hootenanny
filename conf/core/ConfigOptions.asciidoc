...
=== stats.max.values.per.tag.key

* Data Type: int
* Default Value: `5`

The maximum number of unique tag values per key that stats command will display.
...
List of tag keys for which the stats command will display unique tag values and distribution
statistics.  Separate keys with a ';' and surround entire list in double quotes when passing in
