...
Maximum number of lines to keep in memory when serializing the temporary file output just before
writing SQL records to an OSM API database.  Each line represents a separate SQL record.
...
Count at which the temporary file output status of each element type should be logged when writing
to temporary files that are to be loaded into an OSM API database.  e.g. With a value of 100, you
will see a load status update message logged for every 100 temporary file lines written.
...
Use this option if you wish to retain the temporary data generated during an OSM API database write.
...
populated, any temporary data files generated as a result of executing the hoot convert command
...
written to the SQL output for later execution.  If the output destination is a CSV file, the user is
responsible for executing their own record ID sequence update SQL statements against the database.
