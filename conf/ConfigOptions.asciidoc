...
=== poi.polygon.address.score.threshold

* Data Type: double
* Default Value: `1.0`

The minimum similarity the address scores a POI and polygon can have and be considered an address
...
If false, POI to polygon conflation will not take address tags into account during matching.
Disable this setting to boost runtime performance if your data has little or no address information
or you know the address information to be inaccurate.
...
for match evidence.  This setting allows for conflation customization but can be runtime expensive
when enabled.
...
incorrect matches.  This setting is fairly experimental in that it has not been tested with many
datasets, and it can be runtime expensive when enabled.
