...
=== preserve.accuracy.tag

* Data Type: bool
* Default Value: `false`

If true, then the OSM "accuracy" tag is still used to populate circular error by OSM readers but
it is not dropped from output.  Useful in preserving an original tag state for elements when
constructing an I/O chain whose final target is a non-hoot data store (e.g. OSM API database).
...
=== hootapi.db.writer.osmapidb.url

* Data Type: string
* Default Value: ``

This is required when using the OsmApiDbAwareHootApiDbWriter.  It makes that Hoot API database writer
use a specified OSM API database for determining new element ID's.
