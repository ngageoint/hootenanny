...
=== log.identical.message.limit

* Data Type: double
* Default Value: `3`

The number of identical log messages that will be emitted before they are silenced.  Not necessarily
utilized by all parts of the application.
...
must be used in conjunction with the "osmapidb.id.aware.url" setting.  IMPORTANT: This setting
should only be enabled in situations where a single user is writing to the target OSM API database
(database is offline).  Since the command does not continually check for the next available ID
(for performance reasons), ID conflict errors may occur if multiple users are writing to the
database simultaneously when this option is enabled.
...
If the script run longer than the specified time, it is forcefully stopped.  If the value is set to
-1, then there is no time limit.  This is useful when debugging tests which may hang on a remote
build server.
