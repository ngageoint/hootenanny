...
=== conflate.use.data.source.ids.input.1

* Data Type: bool
* Default Value: `false`

Determines whether the conflate command will use the element ID's specified by the input dataset
(true) or use internal ID management for the first conflation input.

=== conflate.use.data.source.ids.input.2
...
=== osm.changeset.file.writer.generate.new.ids

* Data Type: bool
* Default Value: `true`

If true, the OSM changeset file writers will generate new ID's for elements being created.
...
Maximum number of elements (nodes, ways, and relations) that can be held in memory before the
system starts using disk-based storage.  Note that this limit applies to each element type
individually, so worst case is that three times this value of elements can be stored in memory
at the same time.
...
=== hootapi.db.writer.osmapidb.id.sequence.url

* Data Type: string
* Default Value: ``

If not empty, the Hoot API database writer will use a specified OSM API database for determining
new element ID's.  If empty, the Hoot API database writer will use its internal ID sequencing
scheme.  This setting should be used with with new Hoot API datasets only, as turning the setting
on/off when writing to existing datasets can cause element ID conflicts to occur.
...
If true, the Hoot API services database writer will internally remap element ID's.  If false,
the original element ID's will be preserved.
