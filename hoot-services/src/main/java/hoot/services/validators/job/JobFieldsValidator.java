...
public class JobFieldsValidator {
	...
	/*private boolean _validateDoubleStr(String rangeValStr, String fieldValStr, String comparator)
	{
		...
		if(rangeValStr != null)
		{
			...
			try
			{
				rangeVal = Double.parseDouble(rangeValStr);
			}
			catch(NumberFormatException nex)
			{
				// sice we can not determine what to compare so
				return true;
			}

			try
			{
				fieldVal = Double.parseDouble(fieldValStr);
			}
			catch(NumberFormatException nex)
			{
				return false;
			}
			double diff = Math.abs(rangeVal - fieldVal);
			boolean isDiffSignificant = false;
			...
			if(comparator.equals("<"))
			{
				...
				if(fieldVal < rangeVal && isDiffSignificant)
				{
					ret = false;
				}
			}
	...
	/*private boolean _validateIntStr(String rangeValStr, String fieldValStr, String comparator)
	{
		...
		if(rangeValStr != null)
		{
			...
			try
			{
				rangeVal = Integer.parseInt(rangeValStr);
			}
			catch(NumberFormatException nex)
			{
				// sice we can not determine what to compare so
			...
			try
			{
				fieldVal = Integer.parseInt(fieldValStr);
			}
			catch(NumberFormatException nex)
			{
				return false;
