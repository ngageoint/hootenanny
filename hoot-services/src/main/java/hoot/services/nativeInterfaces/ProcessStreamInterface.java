 ...
 * @author Jong Choi
 *
 * The purpose of this class is to run command line execution in a separate process. It inherit from INativeInterface
 * so it can be switched out with any other class implementing INativeInterface if needed.
 * It is called by jobExecutionManager and it gets associated through spring entry located in CoreServiceContext.xml.
 * This class handles 2 types of execution format.
 * 1. direct exec call like hoot --ogr2osm target input output if the "exectype" is "hoot"
 * 2. make file script based execution where format is make -f [some makefile] [any argument make file uses] when "exectype" is "make"
 *
 */
public class ProcessStreamInterface implements INativeInterface {
	...
	/* (non-Javadoc)
	 * @see hoot.services.nativeInterfaces.INativeInterface#exec(org.json.simple.JSONObject)
	 */
	@Override
