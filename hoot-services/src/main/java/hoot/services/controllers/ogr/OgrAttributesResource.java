...
public class OgrAttributesResource extends JobControllerBase
{
	...
	/**
	 * This rest endpoint uploads multipart data from UI and then generates attribute output Example:
	 * http://localhost:8080//hoot-services/ogr/info/upload?INPUT_TYPE=DIR Output:
	 * {"jobId":"e43feae4-0644-47fd-a23c-6249e6e7f7fb"}
	 * 
	 * After getting the jobId, one can track the progress through job status rest end point Example:
	 * http://localhost:8080/hoot-services/job/status/e43feae4-0644-47fd-a23c-6249e6e7f7fb Output:
	 * {"jobId":"e43feae4-0644-47fd-a23c-6249e6e7f7fb","statusDetail":null,"status":"complete"}
	 * 
	 ...
	 * 
	 * @param inputType
	 *          : [FILE | DIR] where FILE type should represents zip,shp or OMS and DIR represents
	 *          FGDB
	 */
	@POST
	@Path("/upload")
			...
			@Context HttpServletRequest request)
	{
		...
		try
		{
			...
			while (it.hasNext())
			{
				...
				// If so then we add the folder location and desired output name which is fgdb name in the
				// zip
		...
		{
			ResourceErrorHandler.handleError("Failed upload: " + ex.toString(),
	 ...
	 * 
	 *          Example:http://localhost:8080/hoot-services/ogr/info/e43feae4-0644-47fd-a23c-6249e6
	 *          e7f7fb output: JSON of attributes
	 * 
			...
			@QueryParam("deleteoutput") String doDelete)
	{
		...
		catch (Exception ex)
		{
			ResourceErrorHandler.handleError(
					"Error getting attribute: " + id + " Error: " + ex.getMessage(),
					Status.INTERNAL_SERVER_ERROR, log);
		}
