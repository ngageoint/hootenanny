...
public class ElementResource {
    ...
    throws Exception {
        ...
        try {
            ...
            elementDoc = getElementXml(elementIdParts[0], Long.parseLong(elementIdParts[2]), elementTypeVal, true, false, conn);
        }
        finally {
            DbUtils.closeConnection(conn);
        }
    ...
    throws Exception {
        ...
        try {
            ...
            elementDoc =
                    getElementXml(elementIdParts[0], Long.parseLong(elementIdParts[2]), elementType, true, true, conn);
        }
        finally {
            DbUtils.closeConnection(conn);
        }
    ...
    private static Document getElementXml(String mapId, long elementId,
                                          ElementType elementType, boolean multiLayerUniqueElementIds,
                                          boolean addChildren, Connection dbConn)
    throws Exception {
        long mapIdNum = -1;
        try {
        ...
        List<Tuple> elementRecords = (List<Tuple>) Element.getElementRecordsWithUserInfo(mapIdNum, elementType, elementIds, dbConn);
        ...
        Element element = ElementFactory.getInstance().create(elementType, elementRecords.get(0), dbConn, Long.parseLong(mapId));
