...
public class ZCurveRanger
{
  ...
  /**
   * Find a good break point for the given box based on major z-value breaks and
   * break the box into two children.
   */
  LongBox[] _breakBox(LongBox box)
  {
    ...
    for (int d = 0; d < _zv.getDimensions(); d++)
    {
      ...
      // the later dimensions have a more significant bits so we'll split on >=
   ...
   * Denser data sets probably want a smaller value. Less dense data sets should
   * have a higher value.
   * 
   * @param slop
   */
