...
public abstract class WpsTestAbstract extends OsmResourceTestAbstract
{
  ...
  protected String testWps(final boolean kvp, final boolean getRequest, final String mapId) 
    throws Exception
  {
    final HttpServletRequest request = Mockito.mock(HttpServletRequest.class);   
    Mockito
      .when(request.getRequestURL())
      .thenReturn(new StringBuffer("http://localhost:8080/hoot/ogc"));
    ...
    if (getRequest)
    {
      ...
      else
      {
        ...
        //queryString = /*StringEscapeUtils.escapeXml(*/queryString/*)*/;
    ...
    final String errorMessage) throws Exception
  {
    ...
    try
    {
      ...
      Assert.assertEquals(
        errorStatus.getStatusCode() + ": " + errorStatus.getReasonPhrase(), 
        xpath.evaluate(
          ".//Status/ProcessFailed/ExceptionReport/Exception/@exceptionCode", responseData));
      Assert.assertTrue(
        xpath.evaluate(
          ".//Status/ProcessFailed/ExceptionReport/Exception/ExceptionText", responseData)
