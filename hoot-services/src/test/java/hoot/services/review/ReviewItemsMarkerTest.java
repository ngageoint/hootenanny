...
public class ReviewItemsMarkerTest {
	...
	public void testGetLastAccessUpdateClause() throws Exception
	{
  	...
  	Timestamp newLastAccessTime = new Timestamp(date.getTime());
		SQLUpdateClause cl = spy._getLastAccessUpdateClause("{123456789}", newLastAccessTime);
	...
	public void testUpdateLastAccessWithSubSelect() throws Exception
	{
  	...
  	SQLUpdateClause q = spy._updateLastAccessWithSubSelect(now, compareTime, "{123456789}");
		
		Assert.assertEquals("update \"review_items\"\n" + 
				"set \"last_accessed\" = ?\n" + 
				"where \"review_items\".\"map_id\" = ? and \"review_items\".\"reviewable_item_id\" in (select \"review_items\".\"reviewable_item_id\"\n" + 
