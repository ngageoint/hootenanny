...
public:
  ...
  static void crop(OsmMapPtr map, const Envelope& envelope);

  static void crop(OsmMapPtr map, const boost::shared_ptr<const Geometry>& g,
    bool invert);
...
private:
  ...
  void _cropWay(OsmMapPtr map, long wid);
   ...
   */
  long _findNodeId(ConstOsmMapPtr map, ConstWayPtr w, const Coordinate& c);
  ...
  WayPtr _reintroduceWay(OsmMapPtr map, ConstWayPtr w,
    const LineString* ls);
