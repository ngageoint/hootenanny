...
void UnconnectedWaySnapper::setConfiguration(const Settings& conf)
{
  ...
  else  // Setting it to zero, if it wasn't specified, allows for skipping way node reuse completely.
...
void UnconnectedWaySnapper::setMaxNodeReuseDistance(double distance)
{
  if (distance <= 0.0)
  {
    throw IllegalArgumentException(
      QString("Invalid %1 value: %2. The value must be greater than 0.0.")
        .arg(ConfigOptions::getSnapUnconnectedWaysExistingWayNodeToleranceKey(), QString::number(distance)));
  }
...
void UnconnectedWaySnapper::setMaxSnapDistance(double distance)
{
  if (distance <= 0.0)
  {
    throw IllegalArgumentException(
      QString("Invalid %1 value: %2. The value must be greater than 0.0.")
        .arg(ConfigOptions::getSnapUnconnectedWaysSnapToleranceKey(), QString::number(distance)));
  }
...
void UnconnectedWaySnapper::setWayDiscretizationSpacing(double spacing)
{
  if (spacing <= 0.0)
  {
    throw IllegalArgumentException(
      QString("Invalid %1 value: %2. The value must be greater than 0.0.")
        .arg(ConfigOptions::getSnapUnconnectedWaysDiscretizationSpacingKey(), QString::number(spacing)));
  }
...
void UnconnectedWaySnapper::setMinTypeMatchScore(double score)
{
  if (score != -1.0 && (score <= 0.0 || score > 1.0))
    throw IllegalArgumentException("Minimum type match score must be greater than 0 and less than or equal to 1 (-1 to disable).");
...
void UnconnectedWaySnapper::apply(OsmMapPtr& map)
{
  ...
  LOG_DEBUG("Searching for ways to snap out of " << StringUtils::formatLargeNumber(ways.size()) << " total ways...");
  ...
  for (auto wayItr = ways.begin(); wayItr != ways.end(); ++wayItr)
  {
    ...
    if (_conflateInfoCache && !_conflateInfoCache->elementCanBeConflatedByActiveMatcher(wayToSnap, className()))
    {
      LOG_TRACE(
    ...
    {
      LOG_TRACE("Skipping processing of " << wayToSnap->getElementId() << ", as it is of an explicitly excluded type.");
      continue;
    }
...
void UnconnectedWaySnapper::_createAllFeatureCriteria()
{
  ...
  if (_snapToExistingWayNodes)
  {
   LOG_DEBUG("Creating way node to snap to criteria...");
    _wayNodeToSnapToCrit = _createFeatureCriteria(_wayToSnapToCriteria, _snapToWayStatuses, true);
  }
...
void UnconnectedWaySnapper::_createFeatureIndexes()
{
  ...
  LOG_DEBUG("Creating crossing way node index...");
  std::shared_ptr<WayNodeCriterion> nodeCrit = std::make_shared<WayNodeCriterion>(_map);
  _createFeatureIndex(nodeCrit, _crossingWayNodeIndex, _crossingWayNodeIndexToEid, ElementType::Node);
}
...
ElementCriterionPtr UnconnectedWaySnapper::_createFeatureCriteria(const QStringList& typeCriteria,
...
ElementCriterionPtr UnconnectedWaySnapper::_getTypeCriteria(const QStringList& typeCriteria,
                                                            const bool isNode) const
{
  if (typeCriteria.isEmpty())
  ...
  else
  {
    ...
    for (const auto& crit : qAsConst(typeCriteria))
      orCrit->addCriterion(_getTypeCriterion(crit, isNode));
...
ElementCriterionPtr UnconnectedWaySnapper::_getTypeCriterion(const QString& typeCriterion,
                                                             const bool isNode) const
{
  ...
  if (!typeCriterion.trimmed().isEmpty())
  {
    ...
    std::shared_ptr<ConflatableElementCriterion> conflatableCrit = std::dynamic_pointer_cast<ConflatableElementCriterion>(typeCrit);
    if (!conflatableCrit)
      throw IllegalArgumentException("Only classes inheriting from ConflatableElementCriterion are valid as way snapping criteria.");
    if (conflatableCrit->getGeometryType() != GeometryTypeCriterion::GeometryType::Line)
      throw IllegalArgumentException("Only classes capable of conflating linear features are valid as way snapping criteria.");
    ...
    // all linear crits.
    if (isNode)
      typeCrit = Factory::getInstance().constructObject<ElementCriterion>(conflatableCrit->getChildCriteria().at(0));

    // configure our element criterion, in case it needs it
    std::shared_ptr<Configurable> configurable = std::dynamic_pointer_cast<Configurable>(typeCrit);
    if (configurable)
      configurable->setConfiguration(_conf);

    std::shared_ptr<ConstOsmMapConsumer> mapConsumer = std::dynamic_pointer_cast<ConstOsmMapConsumer>(typeCrit);
    if (mapConsumer)
...
ElementCriterionPtr UnconnectedWaySnapper::_getStatusCriteria(const QStringList& statuses) const
{
  ...
  {
    std::shared_ptr<OrCriterion> orCrit = std::make_shared<OrCriterion>();
    for (const auto& status : qAsConst(statuses))
      orCrit->addCriterion(std::make_shared<StatusCriterion>(Status::fromString(status)));
    statusCrit = orCrit;
  }
  LOG_VART(statusCrit);
  return statusCrit;
                                                ...
                                                const ElementType& elementType)
{
  ...
  featureIndex = std::make_shared<Tgs::HilbertRTree>(std::make_shared<Tgs::MemoryPageStore>(728), 2);
...
void UnconnectedWaySnapper::_snapUnconnectedWayEndNodes(const WayPtr& wayToSnap)
{
  ...
  const std::set<long> unconnectedEndNodeIds = _getUnconnectedWayEndNodeIds(wayToSnap, _unconnectedWayNodeCrit);
  for (auto unconnectedEndNodeId : unconnectedEndNodeIds)
  {
...
bool UnconnectedWaySnapper::_snapUnconnectedWayEndNode(const NodePtr& unconnectedEndNode, const WayPtr& wayToSnap)
{
...
{  // Only snap 1 to 2, no need to snap 2 to 1
  ...
  // Get envelop of our way
  std::shared_ptr<geos::geom::Envelope> env1(wayToSnap->getEnvelope(_map));
                                                                   ...
                                                                   const ElementCriterionPtr& wayCrit) const
{
  ...
  const std::set<long> filteredWaysContainingFirstEndNode = WayUtils::getContainingWayIds(firstEndNodeId, _map, wayCrit);
  const std::set<long> filteredWaysContainingSecondEndNode = WayUtils::getContainingWayIds(secondEndNodeId, _map, wayCrit);
...
QList<ElementId> UnconnectedWaySnapper::_getNearbyFeaturesToSnapTo(const ConstNodePtr& node,
                                                                   const ElementType& elementType) const
{
  ...
  if (elementType == ElementType::Node)
  {
    ...
    neighborIds = SpatialIndexer::findSortedNodeNeighbors(node, *env, _snapToWayNodeIndex, _snapToWayNodeIndexToEid, _map);
  }
  else
  {
    env->expandBy(_getWaySearchRadius(node));
    const std::set<ElementId> neighborIdsSet =
      SpatialIndexer::findNeighbors(*env, _snapToWayIndex, _snapToWayIndexToEid, _map, elementType, false);
    for (const auto& elementId : neighborIdsSet)
      neighborIds.append(elementId);
  }
  LOG_VART(neighborIds);
  return neighborIds;
}

bool UnconnectedWaySnapper::_snapUnconnectedWayEndNodeToWayNode(const NodePtr& nodeToSnap,
                                                                const Tags& wayToSnapTags)
{
  ...
  for (const auto& elementId : qAsConst(wayNodesToSnapTo))
  {
    const long wayNodeToSnapToId = elementId.getId();
    ...
    if (wayNodeToSnapToId != nodeToSnap->getId())
    {
      ...
      if (!WayUtils::nodesAreContainedInTheSameWay(wayNodeToSnapToId, nodeToSnap->getId(), _map))
      {
        ...
        {
          const std::vector<ConstWayPtr> containingWays = WayUtils::getContainingWaysConst(wayNodeToSnapToId, _map);
          ...
          {   
            ...
            if (schema.explicitTypeMismatch(wayToSnapTags, containingWay->getTags(), _minTypeMatchScore))
            {
              LOG_TRACE(
        ...
        else
        {
          LOG_TRACE(
            ...
            elementId.getId() << " and " << nodeToSnap->getId() <<
...
void UnconnectedWaySnapper::_snap(const NodePtr& nodeToSnap, const NodePtr& wayNodeToSnapTo)
{
  ...
  const std::set<long>& waysContainingWayNodeToSnapTo = _map->getIndex().getNodeToWayMap()->getWaysByNode(wayNodeToSnapTo->getId());
  ...
  if (!_markOnly)
  {
    LOG_TRACE("Replacing " << nodeToSnap->getElementId() << " with " << wayNodeToSnapTo->getElementId());
    // Merge the tags.
    wayNodeToSnapTo->setTags(TagMergerFactory::mergeTags(wayNodeToSnapTo->getTags(), nodeToSnap->getTags(), ElementType::Node));
    // Replace the snapped node with the node we snapped it to.
    ReplaceElementOp(nodeToSnap->getElementId(), wayNodeToSnapTo->getElementId(), true).apply(_map);
  }
...
bool UnconnectedWaySnapper::_snapUnconnectedWayEndNodeToWay(const NodePtr& nodeToSnap, const Tags& wayToSnapTags)
{
  if (!nodeToSnap)
    return false;
  ...
  // get nearby ways
  const QList<ElementId> waysToSnapTo = _getNearbyFeaturesToSnapTo(nodeToSnap, ElementType::Way);
  for (const auto& elementId : qAsConst(waysToSnapTo))
  {
    // for each neighboring way
    WayPtr wayToSnapTo = _map->getWay(elementId.getId());
    ...
    if (!wayToSnapTags.isEmpty())
    {
      ...
      else if (schema.explicitTypeMismatch(wayToSnapTags, wayToSnapTo->getTags(), _minTypeMatchScore))
      {
        LOG_TRACE(
    ...
    const bool nodeToSnapContainedByWaySharingNodesWithWayToSnapTo =
      WayUtils::nodeContainedByWaySharingNodesWithAnotherWay(nodeToSnap->getId(), wayToSnapTo->getId(), _map);
...
bool UnconnectedWaySnapper::_snapUnconnectedWayEndNodeToWay(const NodePtr& nodeToSnap, const WayPtr& wayToSnapTo,
                                                            bool& snappedToNode)
{
  ...
  std::vector<long> wayNodeIdsToSnapTo = wayToSnapTo->getNodeIds();
  ...
  if (!wayToSnapToContainsNodeToSnap)
  {
    ...
    if (shortestDistanceFromNodeToSnapToWayCoord <= _maxSnapDistance)
    {
      ...
      // optimization.
      NodePtr closestWayNode = _map->getNode(WayUtils::closestWayNodeIdToNode(nodeToSnap, wayToSnapTo, _map));
      const geos::geom::Coordinate closestWayCoordinate = closestWayNode->toCoordinate();
      const double distanceBetweenWaySnapCoordAndClosestWayNode = Distance::euclidean(closestWayToSnapToCoord, closestWayCoordinate);
      LOG_VART(distanceBetweenWaySnapCoordAndClosestWayNode);
      if (distanceBetweenWaySnapCoordAndClosestWayNode <= ConfigOptions().getDuplicateNodeRemoverDistanceThreshold())
      {
      ...
      const long nodeToSnapInsertIndex = WayUtils::closestWayNodeInsertIndex(nodeToSnap, wayToSnapTo, _map);
      ...
      if (Log::getInstance().getLevel() <= Log::Debug)
      {
        ...
        const geos::geom::Coordinate wgs84Coord =
          MapProjector::project(closestWayToSnapToCoord, _map->getProjection(), MapProjector::createWgs84Projection());
        LOG_TRACE(
      ...
      if (!_markOnly)
      {
        ...
        wayNodeIdsToSnapTo.insert(wayNodeIdsToSnapTo.begin() + nodeToSnapInsertIndex, nodeToSnap->getId());
        wayToSnapTo->setNodes(wayNodeIdsToSnapTo);
...
bool UnconnectedWaySnapper::snapClosestWayEndpointToWay(OsmMapPtr map, const WayPtr& disconnected, const WayPtr& connectTo)
{
  LOG_TRACE("Attempting to snap " << disconnected->getElementId() << " to " << connectTo->getElementId() << "...");
...
bool UnconnectedWaySnapper::_snapClosestWayEndpointToWay(const WayPtr& disconnected, const WayPtr& connectTo)
{
  //  Validate the parameters
...
{
  std::set<long> owningWayIds = WayUtils::getContainingWayIds(idOfNodeBeingSnapped, _map);
...
void UnconnectedWaySnapper::_reviewSnappedWay(const long idOfNodeBeingSnapped) const
{
  std::set<long> owningWayIds = WayUtils::getContainingWayIds(idOfNodeBeingSnapped, _map);
  const long owningWayId = *owningWayIds.begin();
  _reviewMarker.mark(_map, _map->getWay(owningWayId), "Potentially snappable unconnected way", className(), 1.0);
}
