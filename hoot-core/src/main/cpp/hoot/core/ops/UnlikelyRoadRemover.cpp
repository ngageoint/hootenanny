...
ElementCriterionPtr UnlikelyRoadRemover::_getRemovalCrit(const ConstOsmMapPtr& map) const
{
  ...
  std::shared_ptr<WayLengthCriterion> lengthCrit =
    std::make_shared<WayLengthCriterion>(_maxWayLength, NumericComparisonType::LessThanOrEqualTo, map);
  ...
  std::shared_ptr<WayHeadingVarianceCriterion> headingCrit =
    std::make_shared<WayHeadingVarianceCriterion>(_maxHeadingVariance, NumericComparisonType::GreaterThanOrEqualTo, map);
...
void UnlikelyRoadRemover::apply(OsmMapPtr& map)
{
  ...
  // Remove the ways.
  for (auto way_id : wayIdsToRemove)
  {
    RecursiveElementRemover(ElementId::way(way_id)).apply(map);
    _numAffected++;
  }
