...
WaySublineMatchStringPtr WayMatchStringMerger::createMatchString() const
{
  ...
  return std::make_shared<WaySublineMatchString>(matches);
}
                                              ...
                                              const WaySubline& subline2)
{
  ...
  SublineMappingPtr lastSm = std::make_shared<SublineMapping>();
...
void WayMatchStringMerger::_moveNode(ElementId scrapNodeId, const WayLocation& wl1)
{
  ...
  NodePtr placeholder =
    std::make_shared<Node>(
      scrapNode->getStatus(), _map->createNextNodeId(), scrapNode->getX(), scrapNode->getY(),
      scrapNode->getCircularError());
...
void WayMatchStringMerger::replaceScraps()
{
  ...
  for (auto it = w2ToW1.begin(); it != w2ToW1.end(); ++it)
  {
...
{
  //  Go through all the way mappings and set the new status.
  for (const auto& sm : qAsConst(_sublineMappingOrder))
...
WayLocation WayMatchStringMerger::_snapToEnd(const WayLocation& wl) const
{
  ...
  else if (static_cast<size_t>(wl.getSegmentIndex()) == wl.getWay()->getNodeCount() - 1 &&
           wl.getSegmentFraction() >= 1 - WayLocation::SLOPPY_EPSILON)
  {
    result = WayLocation(wl.getMap(), wl.getWay(), static_cast<int>(wl.getWay()->getNodeCount() - 1), 1.0);
  }
  else
    result = wl;
