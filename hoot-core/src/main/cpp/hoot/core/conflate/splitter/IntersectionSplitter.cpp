...
IntersectionSplitter::IntersectionSplitter(OsmMapPtr map)
{
  _map = map;
}

void IntersectionSplitter::_mapNodesToWay(WayPtr way)
{
  long wId = way->getId();
...
void IntersectionSplitter::_removeWayFromMap(WayPtr way)
{
  long wId = way->getId();
...
void IntersectionSplitter::splitIntersections(OsmMapPtr map)
{
...
void IntersectionSplitter::_splitWay(long wayId, long nodeId)
{
  ...
  if (firstIndex != -1)
  {
    ...
    if (concurrent_count < otherWays_count)
    {
      ...
      vector< WayPtr > splits = WaySplitter::split(_map, way, wl);
...
void IntersectionSplitter::apply(OsmMapPtr& map)
{
