                                     ...
                                     long& nodeCount, long& wayCount)
{
  ...
  if (!wayIds.empty())
  {
    ...
    std::shared_ptr<QSqlQuery> wayItr = _getDatabase()->selectElementsByElementIdList(wayIds, TableType::Way);
    const bool tagConnectedWays = ConfigOptions().getBoundsTagImmediatelyConnectedOutOfBoundsWays();
    ...
    std::shared_ptr<QSqlQuery> additionalWayNodeIdItr = _getDatabase()->selectWayNodeIdsByWayIds(wayIds);
...
void ApiDbReader::_readByBounds(OsmMapPtr map, const Envelope& bounds)
{
  ...
  if (!_returnNodesOnly && !nodeIds.empty())
  {
    ...
    std::shared_ptr<QSqlQuery> relationIdItr = _getDatabase()->selectRelationIdsByMemberIds(nodeIds, ElementType::Node);
    ...
    while (!relationIds.empty())
    {
      ...
      std::shared_ptr<QSqlQuery> relationItr = _getDatabase()->selectElementsByElementIdList(relationIds, TableType::Relation);
      while (relationItr->next())
      {
        ...
        {
          ElementType type = member.getElementId().getType();
          QString id = QString::number(member.getElementId().getId());
      ...
      if (!newNodes.empty())
      {
        std::shared_ptr<QSqlQuery> queryItr = _getDatabase()->selectElementsByElementIdList(newNodes, TableType::Node);
        while (queryItr->next())
        {
      ...
      if (!newWays.empty())
      {
        ...
        LOG_DEBUG("Retrieving ways by way ID...");
        std::shared_ptr<QSqlQuery> wayItr = _getDatabase()->selectElementsByElementIdList(newWays, TableType::Way);
        while (wayItr->next())
        {
          WayPtr way = _resultToWay(*wayItr, *map);
          map->addElement(way);
        ...
        LOG_DEBUG("Retrieving way node IDs referenced by the selected ways...");
        std::shared_ptr<QSqlQuery> additionalWayNodeIdItr = _getDatabase()->selectWayNodeIdsByWayIds(newWays);
        while (additionalWayNodeIdItr->next())
        {
          const long nodeId = (*additionalWayNodeIdItr).value(0).toLongLong();
          LOG_VART(ElementId(ElementType::Node, nodeId));
...
void ApiDbReader::_read(OsmMapPtr map, const ElementType& elementType)
{
  ...
  std::shared_ptr<QSqlQuery> elementResultsIterator = _getDatabase()->selectAllElements(elementType);
  ...
  {
    std::shared_ptr<Element> element = _resultToElement(*elementResultsIterator, elementType, *map );
...
std::shared_ptr<Element> ApiDbReader::_getElementUsingIterator()
{
  ...
  std::shared_ptr<Element> element = _resultToElement(*_elementResultIterator, _selectElementType, *_partialMap);
...
std::shared_ptr<Element> ApiDbReader::readNextElement()
{
  ...
  if (hasMoreElements())
  {
    ...
    if (_numRelationsRead % _statusUpdateInterval == 0 && _selectElementType == ElementType::Relation)
    {
...
std::shared_ptr<Element> ApiDbReader::_resultToElement(QSqlQuery& resultIterator, const ElementType& elementType, OsmMap& map)
{
