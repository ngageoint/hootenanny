...
SetTagValueVisitor::SetTagValueVisitor(const QStringList& keys, const QStringList& values, bool appendToExistingValue,
    ...
    _circularErrorTagKeys(ConfigOptions().getCircularErrorTagKeys())
{
  if (_keys.size() != _vals.size())
  {
    throw IllegalArgumentException(
      QString("set.tag.value.visitor keys and values must be the same length. Keys size: %1, values size: %2")
        .arg(QString::number(_keys.size()), QString::number(_vals.size())));
  }
