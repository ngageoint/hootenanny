...
function additiveScore(map, e1, e2) {
    ...
    var searchRadius = Math.max(e1SearchRadius, e2SearchRadius);
    if ((getRelatedTags("railway=station", t1).length > 0 &&
         getRelatedTags("highway=bus_stop", t2).length > 0) ||
        (getRelatedTags("highway=bus_stop", t1).length > 0 &&
         getRelatedTags("railway=station", t2).length > 0))
    {
