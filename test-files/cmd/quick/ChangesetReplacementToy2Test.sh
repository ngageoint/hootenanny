...
hoot changeset-derive-replacement $CONFIG -D changeset.replacement.implementation=hoot::ChangesetReplacementCreator6 -D bounds.output.file=$OUT_DIR/bounds.osm -D debug.maps.write=false -D debug.maps.filename=$OUT_DIR/debug.osm -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D convert.bounding.box.remove.missing.elements=false $IN_DIR/input1.osm $IN_DIR/input2.osm "-115.036717,36.04301,-115.035772,36.044142" $OUT_DIR/output.osc --full-replacement --write-bounds
