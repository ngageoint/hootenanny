...
export HOOT_OPTS="-D reader.add.source.datetime=false -D uuid.helper.repeatable=true -D reader.preserve.all.tags=true -D writer.include.circular.error.tags=false -D changeset.user.id=1 -D osmapidb.bulk.writer.reserve.record.ids.before.writing.data=true -D osmapidb.bulk.writer.validate.data=true -D osmapidb.bulk.writer.output.files.copy.location=$OUTPUT_DIR/DcGisRoads.sql"
