...
OPT_COMMAND=`hoot | grep optimize-network-conflation-options`
#echo $OPT_COMMAND
if [ -z "$OPT_COMMAND" ]; then
  #echo "optimize-network-conflation-options command not present.  Exiting test without running it."
 exit 0
fi

# we're not verifying output here...just making sure the command runs to completion and the output exists
hoot optimize-network-conflation-options --error "test-files/cmd/slow/OptimizeConflateSettingsCmd/static-tests" "case" "test-files/cmd/slow/OptimizeConflateSettingsCmd/testSettings.json" 2 $OUTPUT_DIR/$OUTPUT_FILE "false" "test-files/cases/hoot-rnd/network/Config.conf"
