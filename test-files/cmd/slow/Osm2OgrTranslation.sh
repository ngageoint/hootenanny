...
hoot convert --debug -C Testing.conf test-files/io/SampleTranslation.osm $OUTPUT/output.shp --trans test-files/io/SampleTranslation.js
printLayerInfo $OUTPUT/output/

echo "#### FileGDB Output ####"
hoot convert --debug -C Testing.conf test-files/io/SampleTranslation.osm $OUTPUT/output.gdb --trans test-files/io/SampleTranslation.js
printLayerInfo $OUTPUT/output.gdb/

echo "#### Test prepending layer names and lazy layer creation ####"
hoot convert --debug -C Testing.conf -D ogr.writer.pre.layer.name=bar_ test-files/io/SampleTranslation.osm $OUTPUT/options.shp --trans test-files/io/SampleTranslation.js
hoot convert --debug -C Testing.conf -D ogr.writer.pre.layer.name=foo_ -D ogr.writer.create.all.layers=true test-files/io/SampleTranslation.osm $OUTPUT/options.shp --trans test-files/io/SampleTranslation.js
...
hoot convert --warn -C Testing.conf test-files/cmd/slow/delaware.shp test-output/cmd/slow/delaware-tds.shp --trans test-files/io/O2sTranslation.js
