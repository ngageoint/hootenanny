...
hoot changeset-derive -D hash.seed.zero=true -D changeset.user.id=1 $INPUT_DIR/map1.osm $INPUT_DIR/map2.osm $OUTPUT_DIR/changeset-1.osc.sql $OSM_API_DB_URL
diff $INPUT_DIR_2/changeset-1.osc.sql $OUTPUT_DIR/changeset-1.osc.sql 

hoot changeset-derive -D hash.seed.zero=true -D changeset.user.id=1 $INPUT_DIR/map1.osm "" $OUTPUT_DIR/changeset-2.osc.sql $OSM_API_DB_URL
diff $INPUT_DIR_2/changeset-2.osc.sql $OUTPUT_DIR/changeset-2.osc.sql 

# external merge sorting tests

hoot changeset-derive -D hash.seed.zero=true -D changeset.user.id=1 -D element.sorter.element.buffer.size=5 $INPUT_DIR/map1.osm $INPUT_DIR/map2.osm $OUTPUT_DIR/changeset-3.osc.sql $OSM_API_DB_URL
diff $INPUT_DIR_2/changeset-3.osc.sql $OUTPUT_DIR/changeset-3.osc.sql 

hoot changeset-derive -D hash.seed.zero=true -D changeset.user.id=1 -D element.sorter.element.buffer.size=5 $INPUT_DIR/map1.osm "" $OUTPUT_DIR/changeset-4.osc.sql $OSM_API_DB_URL
