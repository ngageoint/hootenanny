...
# -D log.class.filter='DiffConflator;OsmApiDbSqlChangesetFileWriter;OsmXmlChangesetFileWriter;OsmChangesetFileWriterFactory'
GENERAL_OPTS="--warn -C DifferentialConflation.conf -C NetworkAlgorithm.conf -C Testing.conf -D uuid.helper.repeatable=true -D writer.include.debug.tags=true -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D debug.maps.write=false"
