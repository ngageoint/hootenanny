...
do
    ...
    hoot convert $HOOT_OPT \
      -D duplicate.node.remover.distance.threshold=1.0 \
      -D schema.translation.script=$TA_IN/cali-translation-$schema.js \
      $TA_IN/$TA_FILE.shp $TA_OUT/$TA_FILE-$schema.osm # > tmp/$TA_FILE-$schema.txt
...
hoot convert $HOOT_OPT \
  -D duplicate.node.remover.distance.threshold=1.0 \
  -D schema.translation.script=$TA_IN/cali-translation-tdsv61-fcode.js \
  $TA_IN/$TA_FILE.shp $TA_OUT/$TA_FILE-tdsv61-fcode.osm

# Uncomment this line to update the input files when there is a schema change
#cp $TA_OUT/$TA_FILE-tdsv61-fcode.osm $TA_IN/$TA_FILE-tdsv61.osm

hoot diff $HOOT_OPT --error-limit 20 --ignore-uuid $TA_OUT/$TA_FILE-tdsv61-fcode.osm $TA_IN/$TA_FILE-tdsv61.osm
