...
# initial data load
hoot convert --error -D hootapi.db.writer.create.user=true -D hootapi.db.writer.email=ServiceHootApiDbReaderTest@hoottestcpp.org -D hootapi.db.writer.overwrite.map=true test-files/DcGisRoads.osm "$HOOT_DB_URL/DcGisRoads-ServiceHootApiDbReaderTest"

# unbounded query
hoot convert --error -D hootapi.db.reader.email=ServiceHootApiDbReaderTest@hoottestcpp.org -D convert.ops="hoot::RemoveTagVisitor;hoot::RemoveAttributeVisitor" -D remove.tag.visitor.keys="source:datetime" -D remove.attribute.visitor.types="changeset;timestamp" "$HOOT_DB_URL/DcGisRoads-ServiceHootApiDbReaderTest" $OUTPUT_DIR/output1.osm
