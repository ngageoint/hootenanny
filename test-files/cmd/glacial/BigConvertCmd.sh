...
hoot big-convert --error -D changeset.user.id=1 -D osmapidb.bulk.writer.output.files.copy.location=$LOCAL_TEST_OUTPUT/output.sql -D osmapidb.bulk.writer.file.output.element.buffer.size=1000 -D big.convert.exec.sql.with.mapreduce=true DcGisRoads.osm.pbf $OSM_API_DB_URL
