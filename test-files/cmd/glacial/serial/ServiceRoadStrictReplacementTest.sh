...
# -D writer.include.debug.tags=true -D debug.maps.write=true -D debug.maps.filename=test-output/cmd/glacial/serial/ServiceRoadStrictReplacementTest/debug.osm
GENERAL_OPTS="--warn -D log.class.filter= -D uuid.helper.repeatable=true -D writer.include.debug.tags=true -D debug.maps.write=true -D debug.maps.filename=test-output/cmd/glacial/serial/ServiceRoadStrictReplacementTest/debug.osm -D changeset.xml.writer.add.timestamp=false -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false"
...
# conflation snapping
CHANGESET_DERIVE_OPTS="-D changeset.user.id=1 -D convert.bounding.box=-71.4698,42.4866,-71.4657,42.4902 -D changeset.buffer= -D convert.ops=hoot::RemoveElementsVisitor;hoot::CookieCutterOp;hoot::UnifyingConflator -D remove.elements.visitor.element.criteria=hoot::HighwayCriterion -D remove.elements.visitor.recursive=true -D element.criterion.negate=true -D changeset.reference.keep.entire.features.crossing.bounds=true -D changeset.secondary.keep.entire.features.crossing.bounds=false -D changeset.reference.keep.only.features.inside.bounds=false -D changeset.secondary.keep.only.features.inside.bounds=false -D conflate.use.data.source.ids.1=true -D conflate.use.data.source.ids.2=true"
