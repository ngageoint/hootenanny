...
GENERAL_OPTS="--warn -D log.class.filter=WayJoinerAdvanced;ReplacementSnappedWayJoiner;WayJoiner -D uuid.helper.repeatable=true -D writer.include.debug.tags=true -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D debug.maps.write=false -D debug.maps.filename=$OUT_DIR/debug.osm"
