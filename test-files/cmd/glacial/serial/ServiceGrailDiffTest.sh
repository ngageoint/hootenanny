...
hoot conflate $CONFIG -D debug.maps.filename=$IN_DIR/debug-conflate.osm $OSM_API_DB_URL "$HOOT_DB_URL/$TEST_NAME-sec" $OUT_DIR/diff.osc.sql --differential --include-tags --separate-output
...
hoot changeset-apply $CONFIG -D debug.maps.filename=$IN_DIR/debug-changeset-application.osm $OUT_DIR/diff-tags.osc.sql $OSM_API_DB_URL
