...
# secondary features lying outside the AOI will be included in the output. See related notes in ServiceBuildingReplacementTest.sh.

TEST_NAME=ServicePoiStrictReplacementTest
...
SEC_LAYER_FILE=$IN_DIR/PoiPolygon2.osm
...
PRUNE_AND_CROP_OPTS="-D convert.ops=hoot::RemoveElementsVisitor -D convert.bounding.box=$AOI -D remove.elements.visitor.element.criteria=hoot::PoiCriterion -D remove.elements.visitor.recursive=true -D element.criterion.negate=true"
...
echo "crop and prune"
hoot convert $GENERAL_OPTS $DB_OPTS $PRUNE_AND_CROP_OPTS -D debug.maps.filename=$OUT_DIR/prune-and-crop-ref.osm -D reader.use.data.source.ids=true -D convert.bounding.box.keep.entire.features.crossing.bounds=false -D bounds.output.file=$OUT_DIR/$TEST_NAME-bounds.osm $REF_LAYER $OUT_DIR/$TEST_NAME-ref-cropped.osm --write-bounds
hoot convert $GENERAL_OPTS $DB_OPTS $PRUNE_AND_CROP_OPTS -D convert.bounding.box.keep.entire.features.crossing.bounds=false -D debug.maps.filename=$OUT_DIR/prune-and-crop-sec.osm -D reader.use.data.source.ids=true $SEC_LAYER $OUT_DIR/$TEST_NAME-sec-cropped.osm
...
hoot cookie-cut $GENERAL_OPTS -D debug.maps.filename=$OUT_DIR/cookie-cut.osm -D reader.use.data.source.ids=true -D crop.keep.entire.features.crossing.bounds=false $OUT_DIR/$TEST_NAME-cutter-shape.osm $OUT_DIR/$TEST_NAME-ref-cropped.osm $OUT_DIR/$TEST_NAME-cookie-cut.osm
