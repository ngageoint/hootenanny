...
GENERAL_OPTS="--trace -D log.class.filter=ChangesetWriter;ChangesetDeriver;InBoundsCriterion;RecursiveSetTagValueOp -D uuid.helper.repeatable=true -D writer.include.debug.tags=true -D changeset.xml.writer.add.timestamp=false -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D debug.maps.write=true"
