...
# crossing the AOI will completely replace corresponding reference features crossing the AOI. 
...
GENERAL_OPTS="--debug -D log.class.filter= -D uuid.helper.repeatable=true -D writer.include.debug.tags=true -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D debug.maps.write=true -D debug.maps.filename=$OUT_DIR/debug.osm"
...
echo ""
hoot changeset-derive-replacement -C UnifyingAlgorithm.conf $GENERAL_OPTS $DB_OPTS $CHANGESET_DERIVE_OPTS $REF_LAYER $SEC_LAYER $AOI hoot::HighwayCriterion $OUT_DIR/$TEST_NAME-changeset-1.osc --lenient-bounds --write-bounds
echo ""
echo $CHANGESET_DERIVATION_MSG " (sql changeset out)..."
echo ""
hoot changeset-derive-replacement -C UnifyingAlgorithm.conf $GENERAL_OPTS $DB_OPTS $CHANGESET_DERIVE_OPTS $REF_LAYER $SEC_LAYER $AOI hoot::HighwayCriterion $OUT_DIR/$TEST_NAME-changeset-1.osc.sql $REF_LAYER --lenient-bounds
