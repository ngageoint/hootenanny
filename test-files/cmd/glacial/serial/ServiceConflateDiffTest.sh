...
CONFIG="--warn -C Testing.conf -D writer.include.debug.tags=true -D uuid.helper.repeatable=true -D debug.maps.write=true -D api.db.email=OsmApiDbHootApiDbConflate@hoottestcpp.org -D hootapi.db.writer.create.user=true -D hootapi.db.writer.overwrite.map=true -D changeset.user.id=1"
...
hoot convert $CONFIG -D debug.maps.filename=$IN_DIR/ref-load.osm $REF_FILE $OSM_API_DB_URL
...
hoot convert $CONFIG -D debug.maps.filename=$IN_DIR/sec-load.osm $SEC_FILE "$HOOT_DB_URL/$TEST_NAME-sec"
# pull the sec out for viewing
hoot convert $CONFIG "$HOOT_DB_URL/$TEST_NAME-sec" $OUT_DIR/sec.osm

echo "Conflating ref with sec..."
hoot conflate $CONFIG -D debug.maps.filename=$IN_DIR/conflate.osm $OSM_API_DB_URL "$HOOT_DB_URL/$TEST_NAME-sec" "$HOOT_DB_URL/$TEST_NAME-sec-conflated"
...
hoot changeset-derive $CONFIG -D debug.maps.filename=$IN_DIR/changeset-derivation.osm $OSM_API_DB_URL "$HOOT_DB_URL/$TEST_NAME-sec-conflated" $OUT_DIR/diff.osc.sql $OSM_API_DB_URL
...
hoot changeset-apply $CONFIG -D debug.maps.filename=$IN_DIR/changeset-application.osm $OUT_DIR/diff.osc.sql $OSM_API_DB_URL

echo "Reading out the final map for comparison with expected map..."
hoot convert $CONFIG -D debug.maps.filename=$IN_DIR/final-map.osm $OSM_API_DB_URL $OUT_DIR/changeset-applied.osm
