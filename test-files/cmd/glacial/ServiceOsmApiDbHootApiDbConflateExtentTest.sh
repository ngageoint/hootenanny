...
# then conflate them, and then verify the output extent is correct.  
#
#This is a subset of the ServiceOsmApiDbConflateAllDataTypesTest, which needs to be updated.
...
CROP_BOUNDS="-77.04,38.8916,-77.03324,38.8958" #slightly larger than the DC ellipse
...
echo "Reading osm api/hoot api db data out with the cropped bounds and conflating them..."
# old logic
...
hoot conflate $HOOT_OPTS -D conflate.use.data.source.ids=true -D osm.map.reader.factory.reader=hoot::OsmApiDbAwareHootApiDbReader -D osm.map.writer.factory.writer=hoot::OsmApiDbAwareHootApiDbWriter -D osmapidb.id.aware.url=$OSM_API_DB_URL -D convert.bounding.box=$CROP_BOUNDS $OSM_API_DB_URL "$HOOT_DB_URL/DcTigerRoads" "$HOOT_DB_URL/DcConflated" --stats $OUTPUT_DIR/stats

# We're just verifying the extents with the command output and not running hoot is-match on the output files.  OSM API DB/Hoot API DB 
# conflation output is verified in other tests.
...
# extent of output should be equal to that of the crop bounds plus any ways/relations that fall partially outside of the bounds along with
