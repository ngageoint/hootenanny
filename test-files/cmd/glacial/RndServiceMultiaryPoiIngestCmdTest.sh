...
REF_DIR=test-files/cmd/glacial/RndServiceMultiaryPoiIngestCmdTest
OUTPUT_DIR=test-output/cmd/glacial/RndServiceMultiaryPoiIngestCmdTest
...
HOOT_OPTS="--warn -D hash.seed.zero=true -D uuid.helper.repeatable=true -D reader.add.source.datetime=false -D writer.include.circular.error.tags=false -D api.db.email=OsmApiDbHootApiDbConflate@hoottestcpp.org -D hootapi.db.writer.create.user=true -D max.elements.per.partial.map=2"
