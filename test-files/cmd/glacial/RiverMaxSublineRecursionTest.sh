...
hoot conflate $LOG_LEVEL -C DifferentialConflation.conf -C NetworkAlgorithm.conf -C Testing.conf -D differential.remove.river.partial.matches.as.whole=true -D match.creators=hoot::ScriptMatchCreator,River.js -D merger.creators=hoot::ScriptMergerCreator -D differential.snap.unconnected.roads=true $IN_DIR/Input1.osm $IN_DIR/Input2.osm $OUT_DIR/diff.osc --differential --changeset-stats $OUT_DIR/stats.json --include-tags --separate-output
