...
/*
    OSM+ to "English" TDSv71 conversion script

    This script is the same as the standard "etds" script but uses "tds71" instead of "tds"
*/
...
etds71 = {
    ...
    toEnglish : function(tags, elementType, geometryType)
    {
        ...
        tdsData = tds71.toOgr(tags, elementType, geometryType)
...
//         }
        ...
        if (tdsData.length > 0)
        {
            for (var fNum = 0, fLen = tdsData.length; fNum < fLen; fNum++)
            {
                ...
                {
                    if (val in etds71.rules.engSingle)
                    {
                        if (tdsData[fNum]['attrs'][val] !== undefined)
                        {
                            eAttrs[etds71.rules.engSingle[val]] = tdsData[fNum]['attrs'][val];
                        }
                ...
                translate.applyOne2One(tdsData[fNum]['attrs'], eAttrs, etds71.rules.engEnum, {'k':'v'},[]);

                // Find an FCODE
                if (tFCODE in etds71.rules.fcodeLookup)
                {
                    if (eAttrs['Feature Code'] !== 'Not found')
                    {
                        eAttrs['Feature Code'] = eAttrs['Feature Code'] + ' & ' + tFCODE + ':' + etds71.rules.fcodeLookup[tFCODE]['desc'];
                    }
                    else
                    {
                        eAttrs['Feature Code'] = tFCODE + ':' + etds71.rules.fcodeLookup[tFCODE]['desc'];
                    }
                }
            } // End for tdsData
