...
/*
    OSM+ to "English" TDSv70 conversion script

    This script is the same as the standard "etds" script but uses "tds70" instead of "tds"
*/
...
etds70 = {
    ...
    toEnglish : function(tags, elementType, geometryType)
    {
        ...
        tdsData = tds70.toTds(tags, elementType, geometryType)
...
//         }
        ...
        if (tdsData.length > 0)
        {
            for (var fNum = 0, fLen = tdsData.length; fNum < fLen; fNum++)
            {
                ...
                {
                    if (val in etds70.rules.engSingle)
                    {
                        if (tdsData[fNum]['attrs'][val] !== undefined)
                        {
                            eAttrs[etds70.rules.engSingle[val]] = tdsData[fNum]['attrs'][val];
                        }
                ...
                if (tFCODE in etds70.rules.fcodeLookup)
                {
                    ...
                    {
                        eAttrs['Feature Code'] = eAttrs['Feature Code'] + ' & ' + tFCODE + ':' + etds70.rules.fcodeLookup[tFCODE]['desc'];
                    }
                    else
                    {
                        eAttrs['Feature Code'] = tFCODE + ':' + etds70.rules.fcodeLookup[tFCODE]['desc'];
                    }
                }
            } // End for tdsData
