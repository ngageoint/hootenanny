       ...
       for (var col in attrs)
        {
            ...
            if (col in ggdm30.swapListIn)
            {
                // print('Swapped: ' + ggdm30.swapListIn[i]); // debug
                attrs[ggdm30.swapListIn[col]] = attrs[col];
                delete attrs[col];
                continue;
            }
        ...
        for (var i in ggdm30.closureList)
        {
            if (attrs[i])
            {
                if (attrs[ggdm30.closureList[i][0]] || attrs[ggdm30.closureList[i][1]])
                {
                    continue;
                }
                else
        ...
        else
        {
            ...
            for (var row in ggdm30.fCodeMap)
            {
                for (var val in ggdm30.fCodeMap[row][1])
                {
                    if (llayerName == ggdm30.fCodeMap[row][1][val])
                    {
                        attrs.F_CODE = ggdm30.fCodeMap[row][0];
                        break;
                    }
                }
            }
...
//                 if (tags.highway && attrs.F_CODE == 'AP030')
        ...
        for (var i=0, iLen=ge4meta.length; i < iLen; i++)
        {
            if (tags[ge4meta[i]])
            {
                if (ggdm30.ge4List[tags[ge4meta[i]]])
                {
                    tags[ge4meta[i]] = ggdm30.ge4List[tags[ge4meta[i]]];
                }
                else
                {
...
function initialize()
{
    ...
    if (ggdm30.fcodeLookup == undefined)
    {
        ...
        fcodeCommon.one2one.push.apply(fcodeCommon.one2one,ggdm30.fcodeOne2oneIn);
...
//             print('End Dump FCODE:');
    ...
    if (ggdm30.lookup == undefined)
    {
        ...
        ggdm30.one2one.push.apply(ggdm30.one2one,ggdm30.one2oneIn);

        ggdm30.lookup = translate.createLookup(ggdm30.one2one);
    }
...
function translateToOsm(attrs, layerName, geometryType)
{
   tags = {};  // The final output Tag list
    ...
    translate.applySimpleNumBiased(notUsedAttrs, tags, ggdm30.numBiased, 'forward',[]);
    translate.applySimpleTxtBiased(notUsedAttrs, tags, ggdm30.txtBiased, 'forward');
