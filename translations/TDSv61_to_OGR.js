...
tds61 = {
	...
	print('toOgr: Init()');
		...
		{
		    tds61.ge4Lookup = {};
		    for (var i in tds61.ge4List)
		    {
		        tds61.ge4Lookup[tds61.ge4List[i]] = i;
		    }
		}
...
// #####################################################################################################
        ...
        if (attrList != undefined)
        {
            ...
            if (tds61.configOut.OgrDebugDumpvalidate == 'true')
            {
        	    for (var val in attrs)
        	    {
                    ...
                    if (val in tds61.txtLength)
                    {
                        if (attrs[val].length > tds61.txtLength[val])
                        {
                            // First try splitting the attribute and grabbing the first value
                            var tStr = attrs[val].split(';');
                            if (tStr[0].length <= tds61.txtLength[val])
                            {
                                attrs[val] = tStr[0];
                            }
                            else
                            {
                                hoot.logWarn('Validate: Attribute ' + val + ' is ' + attrs[val].length + ' characters long. Truncating to ' + tds61.txtLength[val] + ' characters.');
                                // Still too long. Chop to the maximum length.
                                attrs[val] = tStr[0].substring(0,tds61.txtLength[val]);
                            }
    ...
    manyFeatures: function(geometryType, tags, attrs)
    {
        ...
        for (var i = 0, nFeat = newFeatures.length; i < nFeat; i++)
        {
            ...
            translate.applySimpleNumBiased(newFeatures[i]['attrs'], newFeatures[i]['tags'], tds61.numBiased, 'backward',tds61.intList);
            translate.applySimpleTxtBiased(newFeatures[i]['attrs'], newFeatures[i]['tags'], tds61.txtBiased, 'backward');
...
// #####################################################################################################
    ...
    applyToTdsPostProcessing : function (tags, attrs, geometryType, notUsedTags)
    {
        ...
        if (tds61.swapListOut[attrs.F_CODE])
        {
            for (var i in tds61.swapListOut[attrs.F_CODE])
            {
                if (i in attrs)
                {
                    attrs[tds61.swapListOut[attrs.F_CODE][i]] = attrs[i];
                    delete attrs[i]
                }
            }
        }
...
print('lookup: after init LAP030' + tds61.tables.attrLookup['LAP030']);
	...
	translate.applySimpleNumBiased(attrs, notUsedTags, tds61.numBiased, 'backward',tds61.intList);
	translate.applySimpleTxtBiased(attrs, notUsedTags, tds61.txtBiased, 'backward');
	...
	else // We have a feature
	{
	    ...
	    for (var i = 0, fLen = returnData.length; i < fLen; i++)
	    {
	        ...
	        if (tds61.tables.attrLookup[gFcode.toUpperCase()])
	        {
	            ...
	            {
	                returnData[i]['attrs']['FCSUBTYPE'] = tds61.subtypeList[returnData[i]['attrs']['F_CODE']];
	            }
	            ...
	            {
	                returnData[i]['tableName'] = tds61.thematicGroupList[gFcode];
...
// This is just a wrapper so we can call it from other places
// function initialize()
// {
// 	    print('Main TDSv61 Init');
// 	    tds61.initialize();
// }
