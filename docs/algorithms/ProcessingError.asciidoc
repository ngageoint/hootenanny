...
[options="header"]
|======
| |Number of Points Moved|Number of Pairs Separated|Mean Error (nanodegrees)|Max Error (nanodegrees)|Standard Devation of Error (nanodegrees)
...
[options="header"]
|======
| |Number of Points Moved|Number of Pairs Separated|Mean Error (~um)|Max Error (~um)|Standard Devation of Error (~um)
...
The latitude/longitude data is stored in the database as whole numbers in units
of 100 nanodegrees. Though it has not been tested experimentally it should
provide a similar error profile to the +.osm.pbf+ file format described in the
sections above. All values are stored in the Database in the WGS84/EPSG:4326
projection.

When _Hoot Core_ writes data to the DB the only significant errors introduced
should be with regard to the 100 nanodegree rounding described above. The
100 nanodegree rounding is likely introducing a maximum of about 15mm error.
This has not been tested experimentally. Please see <<SelectingAProjection>> for
more details on how projections are utilized within the core.
...
The posted changeset is processed by the web services and the values are rounded
to 100 nanodegrees before being stored in the database as WGS84/EPSG:4326. The
changeset is already in WGS84/EPSG:4326 so no addition change in projection is
necessary.
