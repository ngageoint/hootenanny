...
2) by the elements circular error (`error:circular` tag) if the circular error is larger than the
configured search radius for the feature
3) by the aforementioned customizable search radii passed in as JSON in the configuration option:
`poi.search.radii`
4) by the aforementioned customizable search radii defined in a file whose location is defined by
the `poi.search.radii.config` configuration option
