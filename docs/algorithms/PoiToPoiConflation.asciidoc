...
tag) and customizable search radii that are defined on a per tag basis. The search radii are defined
in the `conf/core/PoiMatchDistances.json` file and can be modified there for custom use cases. Some
example search radii are below:
