...
* `ResetEnvironment` - In addition to what is reset in `ResetBasic`, this resets the configuration
along with many other internal settings but does not reset the `MatchFactory` class (expensive to do
in the tests; see code for details). This reset option is discouraged overall but necessary for some
tests.
