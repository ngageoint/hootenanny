...
coverage will take into command line and CPPUnit tests in the Hootenanny core code code, as well as Java JUnit tests in the Hootenanny Web Services code when the application is configured --with-services.

To generate Hootenanny Core code coverage results do the following (the extra make after configuration is due to ngageoint/hootenanny#604):
...
Then run the command or commands you want to evaluate:

---------------------
make -j($nproc) coverage
-----------------------

For Hootenanny Core C++ and additionally core C++ services and Hootenanny Java Web Services code coverage support, add the --with-services option (there is currently no way via the 'make' command to separate Hootenanny Core C++ services coverage reporting from Hootenanny Web Services Java coverage reporting):

----------
make clean && ./configure --with-rnd --with-services --with-coverage &&  make -j($nproc) && make -j($nproc) test && make -j($nproc) coverage
-----------

For the C++ code, you will have the coverage results in _$HOOT_HOME/tmp/coverage/*/index.html_.  There is a report for each of the main libraries.  For the Java code, a separate report for each project is output to: _hoot-services/target/site/cobertura/index.html_.  If you want a new clean coverage result the you'll need to do make clean again before you start the run. It is possible to clean just the coverage data, but that'll take a little work in Makefile land (ngageoint/hootenanny#600).

For Hootenanny Core C++, code coverage is supplied @gcov@ and @lcov@, utilities for using GCC to generate coverage results.  Java code coverage is supplied by http://cobertura.github.io/cobertura/[Cobertura] via Maven. 
