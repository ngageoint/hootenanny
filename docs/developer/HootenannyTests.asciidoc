...
* `ResetNone` - Use this when you test requires no environment reset, meaning that no environment 
change made by another test can affect it. Generally, this is for tests that make no use of the 
`OsmMap` class. This is the default and does not need to be explicitly specified.
...
test.
* `ResetAllNoMatchFactory` - In addition to what is reset in `ResetBasic`, this resets the 
configuration along with many other internal settings but does not reset the `MatchFactory` class 
(expensive to do in the tests; see code for details). This reset option is discouraged overall but 
necessary for some tests.
* `ResetAll` - This does all the resetting done in `ResetAllNoMatchFactory` and additionally resets 
the `MatchFactory` class. This is the most resource expensive reset option.
...
==== Core Case Tests (Micro Conflate Tests)
...
==== Running Core Tests in Parallel

Hootenanny can run all Core tests in parallel, except those in the Translation test suite. This is 
accomplished with the `HootTest` option, `--parallel [n]`, where the optional `[n]` specifies the 
number of worker processes to spawn.  Leaving off the `[n]` parameter makes an implicit call to the 
operating system to get the total number of online processing units (The same as +$(nproc)+ ).
--------------
...
=== Hootenanny Specific Test Writing Guidelines
