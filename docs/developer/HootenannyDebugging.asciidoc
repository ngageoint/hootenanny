...
* ssh'd into your vagrant vm change the JAVA_OPTS in the Tomcat configuration file:

**For EL9 (Tomcat 9):**
```
# Edit /etc/tomcat9/tomcat9.conf
...
**For EL7 (Tomcat 8):**
```
# Edit /etc/tomcat8/tomcat8.conf
JAVA_OPTS="-Djava.awt.headless=true -Djava.security.egd=file:/dev/./urandom -Djdk.tls.ephemeralDHKeySize=2048 -Xdebug -Xrunjdwp:transport=dt_socket,address=8585,server=y,suspend=n"
```

* Restart Tomcat (in the vagrant box)
