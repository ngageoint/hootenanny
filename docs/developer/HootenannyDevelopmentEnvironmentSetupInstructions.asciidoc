...
. `sudo` `apt-get` `install` `texinfo` `g++` `libicu-dev` `libqt4-dev`
  ...
  `libjson-spirit-dev` `automake1.11` `protobuf-compiler` `libprotobuf-dev`
  `libqt4-sql-psql` `libgeos++-dev` `swig` `lcov` `tomcat6` `openjdk-6-jdk`
  `openjdk-6-dbg` `maven` `libstxxl-dev` `nodejs-dev` `doxygen` `xsltproc`
  `asciidoc` `pgadmin3` `curl` `npm` `libxerces-c28` `libglpk-dev`
  `libboost-all-dev` `source-highlight` `texlive-lang-all` `graphviz` `w3m`
  `python-setuptools` `python` `python-pip` `git` `postgresql-contrib-9.1`
