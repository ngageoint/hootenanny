...
<xsl:stylesheet version="1.0" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="a">
      <xsl:choose>
        <xsl:when test="contains(@class,'wiki-anchor')">
          <xsl:text>&#xA;[[</xsl:text>
          <xsl:value-of select="translate(@href,'#','')"/>
          <xsl:text>]]&#xA;</xsl:text>
        </xsl:when>
        <!-- sometimes when the url contains a _ asciidoc tries to get cute. 
             make sure it behaves with an explicit link. -->
