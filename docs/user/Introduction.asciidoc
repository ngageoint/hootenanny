...
These are the conflation workflows supported by Hootenanny:
...
To use, pass `HorizontalConflation.conf` to the conflate command (TODO: This should be verified. Its possible that `HorizontalConflation.conf` is only applicable for cookie cutting).
