...
-----
    hoot conflate -C AttributeConflation.conf -C UnifyingAlgorithm.conf \
      ...
      -D remove.elements.visitor.element.criteria=LinearWaterwayCriterion;LinearWaterwayWayNodeCriterion;RelationWithRiverMembersCriterion \
...
-----
    hoot convert -D convert.ops="RemoveElementsVisitor" \
      -D remove.elements.visitor.element.criteria="PolygonCriterion" -D element.criterion.negate=true \ 
...
-----
    hoot count -D in.bounds.criterion.bounds="-77.0551,38.8845,-77.0281,38.9031" -D in.bounds.criterion.strict=true \
      input.osm InBoundsCriterion
-----

==== Count all the POIs in multiple maps:

-----
    hoot count "input1.osm;input2.osm" PoiCriterion
-----

==== Count all elements that are not POIs:

-----
    hoot count -D element.criterion.negate=true "input1.osm;input2.osm" PoiCriterion \
      --all-elements
-----
...
-----
    hoot count -D tag.key.contains.criterion.text="phone" input.osm TagKeyContainsCriterion
-----

==== Count all features which have a name:

-----
    hoot count input.osm HasNameCriterion
-----
...
-----
    ...
    hoot count input.osm ConflatableElementCriterion
-----
...
-----
    hoot tag-distribution input.osm hoot:review:note ReviewRelationCriterion
