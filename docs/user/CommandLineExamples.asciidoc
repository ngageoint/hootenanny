...
==== Bring two maps closer in alignment without merging them, using road intersections as tie-points:

-----
    hoot align input1.osm input2.osm output.osm
-----

==== Bring two maps closer in alignment without merging them, using building centroids as tie-points:

-----
    hoot align -D rubber.sheet.use.buildings=true input1.osm input2.osm output.osm
-----
...
==== Retrieve OSM data around Paris, France from the Overpass API
-----
    #  Download XML data from Overpass
    hoot convert -D bounds=2.277303,48.851684,2.311635,48.864701 \
      "https://overpass-api-url/api/interpreter?data=[out:xml];(node({{bbox}});(._;<;>;););out meta;" output.osm

    #  Download JSON data from Overpass
    hoot convert -D bounds=2.277303,48.851684,2.311635,48.864701 \
      "https://overpass-api-url/api/interpreter?data=[out:json];(node({{bbox}});(._;<;>;););out meta;" output.osm
-----
...
==== Convert an OSM file to a FlatGeobuf file

-----
    hoot convert input.osm output.fgb
-----
...
-----
    hoot convert input.osm output.geojson
-----

==== Convert OSM file to GeoJSON, split output into thematic files (i.e. TransportationGroundCrv, FacilitySrf, etc.)

-----
    hoot convert -D geojson.write.split.file.structure=true \
      -D schema.translation.script=translations/TDSv70.js \
      input.osm output.geojson
-----

==== Convert OSM file to GeoJSON, split output into files based on FCODE
...
==== Write the internal tag schema that Hootenanny uses to a CSV file:

-----
    hoot schema -D tag.printing.format=csv > schema.csv
-----

==== Write the internal tag schema graph relations that Hootenanny uses to an image file:

-----
    hoot schema schema.svg
-----
...
==== Stitch two adjacent cropped maps into one combined map

-----
    hoot stitch input1.osm input2.osm output.osm
-----
...
==== Generate a grid with uniform shaped cells across an entire map:

-----
    hoot task-grid input1.osm input2.osm output.geojson --uniform
-----

==== Generate a grid with regular shaped cells that will fit at most 1000 nodes each for an entire map:

-----
    hoot task-grid input1.osm input2.osm output.geojson --maxNodesPerCell 1000 --node-density
-----

==== Generate a grid with uniform shaped cells across a subset of a map:

-----
    hoot task-grid "-71.4698,42.4866,-71.4657,42.4902" output.geojson --uniform
-----

==== Generate a grid with regular shaped cells that will fit at most 1000 nodes each for a subset of a map:
