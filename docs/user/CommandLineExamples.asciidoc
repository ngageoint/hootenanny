...
===== Conflate two maps into a single map using the first map as a reference (Reference Conflation) using the Unifying Algorithm for road matching:

-----
    hoot conflate -C ReferenceConflation.conf -C UnifyingAlgorithm.conf input1.osm input2.osm output.osm
-----

===== Conflate two maps into a single map using Reference Conflation and the Network Algorithm for road matching:
...
===== Combine two maps into a single map within a polygon area:
