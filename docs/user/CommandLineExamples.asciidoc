...
===== Clean erroneous data from two maps making use of JOSM cleaning capabilities:

-----
    hoot clean -D map.cleaner.transforms++=hoot::JosmMapCleaner input.osm output.osm
-----

===== Clean erroneous data from two maps making use of a subset of JOSM cleaning capabilities via exclusion:

-----
    hoot clean -D map.cleaner.transforms++=hoot::JosmMapCleaner \
      -D map.cleaner.josm.validators.exclude="UntaggedWay;DuplicatedWayNodes" input.osm output.osm
-----

===== Clean erroneous data from two maps making use of a subset of JOSM cleaning capabilities via inclusion:

-----
    hoot clean -D map.cleaner.transforms++=hoot::JosmMapCleaner \
      -D map.cleaner.josm.validators.include="UntaggedWay;DuplicatedWayNodes" input.osm output.osm
-----

===== Run JOSM validation only on a map making use of a subset of JOSM cleaning capabilities via exclusion:

-----
    hoot convert -D convert.ops=hoot::JosmValidator \
      -D map.cleaner.josm.validators.exclude="UntaggedWay;DuplicatedWayNodes" input.osm output.osm
-----

===== Run JOSM validation only on a map making use of a subset of JOSM cleaning capabilities via inclusion:
...
===== Run JOSM validation only on a map:

-----
    hoot convert -D convert.ops=hoot::JosmValidator input.osm output.osm
-----
