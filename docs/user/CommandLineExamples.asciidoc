...
-----
    hoot convert -D convert.ops="hoot::FindStreetIntersectionByName" \
...
===== Remove intra-map duplicates within a single map but skip removing nodes:

-----
    hoot de-duplicate input.osm output.osm --skip-nodes
-----

===== Remove intra-map duplicates within a single map with a node filter:
