...
===== Create a changeset that completely replaces all buildings in one dataset with those in another over an AOI:
...
===== Create a changeset that completely replaces all features in one dataset with those in another over an AOI:
...
===== Create a changeset that completely replaces all buildings and POIs in one dataset with those in another over an AOI:
...
===== Create a changeset that completely replaces all cafes with "Java" in the name in one dataset with those in another over an AOI:
...
-----
    hoot changeset-derive-replacement -D changeset.user.id=1 -D tag.criterion.kvps="amenity=cafe" \
      ...
      --additional-filters "hoot::TagContainsCriterion;hoot::TagCriterion" --chain-additional-filters
-----
