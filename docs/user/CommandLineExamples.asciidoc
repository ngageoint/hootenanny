...
===== Replace all features in the first dataset with buildings from the second dataset with a specified bounds:
...
===== Replace all buildings in the first dataset with buildings from the second dataset with a specified bounds:
...
===== Replace all buildings and POIs in the first dataset with all buildings and POIs from the second dataset within the specified bounds:
...
===== Replace all POIs in the first dataset with schools from the second dataset within the specified bounds (TODO: update):
-----
    hoot changeset-derive-replacement -D tag.criterion.kvps="amenity=school" input1.osm input2.osm "38,-105,39,-104" \
      outputChangeset.osc --geometry-filters "hoot::PoiCriterion --input2-filters "hoot::TagCriterion"
-----

===== Replace all schools in the first dataset with schools from the second dataset within the specified bounds:
-----
    hoot changeset-derive-replacement -D tag.criterion.kvps="amenity=school" input1.osm input2.osm "38,-105,39,-104" \
      outputChangeset.osc --geometry-filters "hoot::PoiCriterion --input1-filters "hoot::TagCriterion" \
      --input2-filters "hoot::TagCriterion"
-----

===== Replace all POIs in the first dataset with cafes that hava "Java" in the name from the second dataset within the specified bounds:

-----
    hoot changeset-derive-replacement -D changeset.user.id=1 -D tag.criterion.kvps="amenity=cafe" \
      -D tag.contains.criterion.kvps='name=Java' input1.osm input2.osm "-122.43204,37.7628,-122.4303457,37.76437" \
      output.osc --geometry-filters "hoot::PoiCriterion" 
      --input2-filters "hoot::TagContainsCriterion;hoot::TagCriterion" --chain-input2-filters
-----
