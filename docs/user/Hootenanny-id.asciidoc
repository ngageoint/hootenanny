...
| t | In review mode with a feature selected, toggles between input/conflated layers
...
[[ReviewRelations]]
==== Viewing Review Relations

After conflating two input datasets and entering review mode, users are presented with a review containing Feature A against Feature B, reflective of the pairwise manner that conflation occurs.  In some cases, Feature A or B (highlighted by either blue/red) may represent a relation such as the case with road reviews involving one or more multiline strings.  However in other cases such as with Pois, the relations represent associated Pois that are also reviewed against the same set of features (listed in the tag `hoot:score:uuid` for a selected feature).  In order to view all of the associated review relations for a given feature, click on either the blue/red highlighted feature and select the palette option "Show Review Relations" or Shift-R. This will display a line between the feature and all of the associated review relations.  Users can scroll through each review pair by clicking on the letter icon, i.e. A-Z, above each line.  
...
Hootenanny supports only a single OSM API database connection at one time.  For system administration details on how to configure Hootenanny
to conflate OSM API database data, see the "Configuring OSM API Database Conflation" section of the Hootenanny Installation Guide.
...
"OSM_API_DB_<database-name>".  Select this layer as the reference layer for your conflation job.
...
Hootenanny will export an OSM changeset that is the difference between the data currently stored in the OSM API database and the conflated 
dataset over the geospatial bounds that the original conflation job occurred within.  
...
There is no way to directly view OSM data from an OSM API database within Hootenanny, since the two systems use different logic to display OSM 
...
from the command line, as explained in the "Hootenanny OSM API Database Conflation Testing Notes" section of the Hooteanny Developer's Guide.
