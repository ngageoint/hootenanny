...
Using a quick example, if a conflation job produces a merged layer with 100 open reviews, the first user (user1) who loads the merged layer automatically locks the unresolved review pair loaded in their browser session. If a second user (user2) then simultaneously loads this layer, a different review will be presented to user2 and the total number of locked features will increase to 2 corresponding to the reviews of user1 and user2s. This number of locked features increases as new users add the same merged layer and then decreases as reviews get resolved or as the time duration that features are locked (~2 minutes) expires.  When a review is locked, it cannot be resolved by anyone other than the user who is reviewing it unless they hit the resolve all remaining reviews. Also note that review status changes are committed to the Hootenanny database on a per-review basis and cannot be undone using the undo/redo buttons.
...
image::user/images/id/hoot_save_translationfile.png[scaledwidth="25%"]
...
image::user/images/id/hoot_bulkimport.png[scaledwidth="25%"]
...
NOTE: To select multiple datasets 
...
image::user/images/id/hoot_manageoptions.png[scaledwidth="25%"]
...
image::user/images/id/hoot_validate_layer.png[scaledwidth="50%"]
