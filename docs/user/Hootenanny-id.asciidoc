...
When conflating data it is inevitable that conflicts may arise due to the ambiguous relationships between matched features. When this occurs, reviewable items are flagged and presented to the user in the UI (or in JOSM via the `"Open in JOSM"` button after initial conflation) through an iterative review process where further user is required. This occurrence of getting reviews during a conflation is often the result of any of the following circumstances:
...
==== Opening data layers in JOSM
Datasets can be opened in JOSM using the `Open Dataset in JOSM` option when right-clicking on a dataset in the `Datasets` panel.  Make sure JOSM is up and running before selecting this option.  The dataset will open in JOSM as an editable osm.xml layer containing tags in the OSM translation schema.

Similarly, Export Jobs can be opened in JOSM using the `open in josm` option.