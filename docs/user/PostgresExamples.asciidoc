...
------
hoot convert -D services.db.writer.email='test@test.com' /hoot/data/DcGISRoads.osm.pbf postgresql://hoot:hoottest@localhost:5432/hoot/DcGISRoads
------

* +-D services.db.writer.email+ - a required email notification parameter for the hoot.json file.  The value should appear in +hoot.json+ with a placeholder email address.
