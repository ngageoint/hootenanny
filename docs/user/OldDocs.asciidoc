...
files. For a more nuts and bolts discussion see the +--convert-ogr2osm+ section.
...
*  +finalize+ - An optional method that gets called after all other methods have
   ...
   translation.
*  +translateToOgr+ - Required by the +--convert-osm2ogr+ command to translate from OSM
   to a custom schema.
*  +translateToOsm+ - Required by the +--convert-ogr2osm+ command to translate from a
...
*  +getDbSchema+ - Required by the +--convert-osm2ogr+ command to get the custom schema
...
Below is about the simplest useful example that supports both +--convert-ogr2osm+ and +--convert-osm2ogr+ . The following sections go into details on how these function are used.
...
------
hoot convert-ogr2osm tmp/SimpleExample.js myoutput.osm myinput1.shp myinput2.shp
------
...
------
hoot convert-osm2ogr tmp/SimpleExample.js myinput.osm myoutput.shp
------
...
This is most commonly accessed through the +--convert-osm2ogr+ command.
...
------
hoot convert-osm2ogr translations/MyTranslation.js input.osm output.shp
------
...
------
hoot convert-osm2ogr translations/Poi.js test-files/conflate/unified/AllDataTypesA.osm foo.csv
------
...
------
hoot convert-ogr2osm translations/Poi.js ConvertedBack.osm foo.vrt
------
...
------
hoot convert-ogr2osm -D "ogr2osm.ops=hoot::MergeNearbyNodes" MyTranslation MyOutput.osm MyInput.shp
------
...
Hootenanny provides a link:$$User_-_--convert-ogr2osm.html$$[--convert-ogr2osm] operation to translate and convert shapefiles into OSM files. If the projection is available for the Shapefile the input will be automatically reprojected to WGS84 during the process. If you do a good job of translating the input data into the OSM schema then Hootenanny will conflate the attributes on your features as well as the geometries. If you do not translate the data properly then you'll still get a result, but it may not be desirable.
...
------
hoot convert-ogr2osm TigerRoads tmp/dc-roads/tiger.osm tmp/dc-roads/tl_2012_11001_roads.shp
------
...
------
hoot convert-ogr2osm DcRoads tmp/dc-roads/dcgis.osm tmp/dc-roads/Streets4326.shp
------
...
------
hoot convert-osm2shp "name,highway,surface,foot,horse,bicycle" tmp/dc-roads/output.osm tmp/dc-roads/output.shp
------
...
------
hoot convert-ogr2osm GpsTrack tmp/MyTracks.osm "$HOME/MyTracks.gpx;tracks" 
------
+
...
hoot convert-osm2shp "hoot:max:movement,hoot:mean:movement,hoot:score,name,foot" tmp/MySnappedTracks.osm tmp/MySnappedTracks.shp
------
...
------
hoot convert-ogr2osm  -D "ogr2osm.ops=hoot::MergeNearbyNodes;hoot::PointsToTracksOp" GpsTrack tmp/MyTracks.osm "$HOME/MyTracks.gpx;track_points" 
------
...
+
----
hoot convert-osm2ogr -D ogr.append.data="true" $HOOT_HOME/translations/TDS.js your_data.osm LTDS_4_0.gdb
----
