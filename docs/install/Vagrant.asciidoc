...
**For Rocky Linux 9 / EL9 (Recommended):**
```
...
```
# For EL9
vagrant up hoot_el9_rpm --provision-with updatehoot
...
**Note:** The default VM (`vagrant up`) uses CentOS 7. To use Rocky Linux 9 (EL9), specify the box name:

    vagrant up hoot_el9

Available Vagrant boxes:
* `default` or `vagrant up` - CentOS 7 preprovisioned box (legacy)
* `hoot_el9` - Rocky Linux 9 box for building from source (recommended)
* `hoot_el9_rpm` - Rocky Linux 9 box with RPM installation (recommended)
* `hoot_centos7` - CentOS 7 box for building from source (legacy)
* `hoot_centos7_rpm` - CentOS 7 box with RPM installation (legacy)
...
**For EL9 (Tomcat 9):**
    ...
    sudo rm -rf /usr/share/tomcat9/webapps/hootenannyid
    sudo rm -rf /usr/share/tomcat9/webapps/hoot-services.war
    sudo rm -rf /usr/share/tomcat9/webapps/hoot-services
