...
See the README.md Installation section.

[[fullinstall]]
== CentOS/RHEL Install Instructions

The Hootenanny install on CentOS/RHEL is a work in progress.  In the near future, an RPM based install will be to
ensure the easiest installation possible.

For now, the script: `scripts/InstallHoot-CentOS-RHEL.sh` may be used to install.  This script requires custom 
