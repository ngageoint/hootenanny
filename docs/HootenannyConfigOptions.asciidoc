...
Configuration settings can dramatically change the operation of Hootenanny. The configuration options are carried through any Hadoop commands and can be set in a configuration file (+$HOOT_HOME/conf/hoot.json+) or specified on the command line. Most of the configuration values are documented in more detail in the following sections.
