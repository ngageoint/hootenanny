...
* +input2+                - OSM data input 2; may be any supported input format or empty (e.g. .osm file or empty string ("") to 
                            derive changeset from one input only).
...
* +--stats+               - Displays changeset statistics and may be optionally followed by an output JSON file name to write 
                            the statistics to. Valid when writing the .osc changeset output format only.
