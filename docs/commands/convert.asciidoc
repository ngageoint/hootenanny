...
* +--cols+   - (optional) Comma delimited list of keys to use for columns, surrounded by quotes.  Valid only when converting from OSM 
                to shape file.  If "--cols" is specified and the its value is empty, the list of columns will be automatically determined 
                based on the most frequently populated tags.
